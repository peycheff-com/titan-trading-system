...
2. **Verify Health**
   ...
   ```bash
   ./scrip../health_check.sh
   ```

3. **Confirm DISARMED State**

   ```bash
   ./scrip../set_trading_mode.sh disarm "Pre-launch safety check" "<operator_id>"
...
4. **ARM When Ready**
   ...
   ```bash
   ./scrip../set_trading_mode.sh arm "Initial launch" "<operator_id>"
   ```
...
```bash
./scrip../set_trading_mode.sh disarm "Manual disarm" "<operator_id>"
```
