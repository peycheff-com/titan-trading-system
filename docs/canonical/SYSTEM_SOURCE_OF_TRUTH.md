...
| **Document SHA256**         | Compute via: `shasum -a 256 docs/canonical/SYSTEM_SOURCE_OF_TRUTH.md` |
...
| `titan-tempo`| grafana/tempo       | 3200, 4318    | Distributed tracing                    |
...
graph TD
    subgraph "Strategy Layer"
        ...
        S2["Phase 2: Hunter<br/>(Port 8082)"]
...
| `npm run start:prod`             | Production startup script                  | `package.json:22` → `scripts/ops/start_production.sh` |
...
# OR
npm run start:prod  # Runs scripts/ops/start_production.sh
```
...
| `TITAN_SIGNALS`   | `titan.signal.>`     | Limits (Legacy)    | `packages/shared/src/messaging/NatsClient.ts:198-205` |
...
| `system.halt`                  | Brain/Operator        | Execution               | Command   | `HaltPayload`                 |
...
| ----------------- | -------------------------------------- | ------------------------------------------- |
| `GlobalHalt`      | Operator command via `system.halt`     | `services/titan-execution-rs/src/circuit_breaker.rs` |
...
| `EVT.EXECUTION.REPORT` | `titan.evt.exec.report.v1` |
...
| Daily drawdown            | Brain             | > 5% (of 7% max)         |
...
- Tempo receiver on `4318` (OTLP/gRPC)
...
| -------------------- | ----------- | -------------------------- | ----------------- |
| `titan.data.*`     | Deprecated  | `titan.data.*`             | **Feb 28, 2026**  |
...
```
359e8fad1636e03ee97fde6572346415fc29f650
```
...
git rev-parse HEAD | grep -q "359e8fad1636e03ee97fde6572346415fc29f650" && echo "✓ Commit matches"
...
**Document Generated**: 2026-02-02T17:20:00Z  
