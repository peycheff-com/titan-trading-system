...
| 🔴 **EMERGENCY** | Max daily loss breached OR `HALT` Cmd | **Flatten All**. Cancel All. |
...
- Monitored by Brain.
- If Equity drops below `StartingEquity - MaxDailyLoss`, a `CMD.SYS.HALT` (`titan.cmd.sys.halt.v1`) is fired.
...
```bash
./scrip../set_trading_mode.sh arm "Post-incident re-arm" "<operator_id>"
```
