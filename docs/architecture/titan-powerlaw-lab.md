...
- **Output**: Generates analysis reports/metrics; does not currently emit `titan.data.submit` events to the Brain.
...
2. **Integration**: Connect to `titan.data.submit` as a "Veto" signal (e.g., "High Tail Risk" -> Reduce Leverage).
