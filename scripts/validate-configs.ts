...
function validateFile(filename: string, schema: any, description: string) {
        ...
        if (file.startsWith("phase") && file.endsWith(".config.json")) {
            ...
            if (availableSchemas.includes(serviceName)) {
                ...
                try {
                    const content = fs.readFileSync(filePath, "utf-8");
                    const data = JSON.parse(content);
                    const result = ConfigValidator.validateServiceConfig(
                        serviceName,
                        data,
                    );
                    if (result.valid) {
                        logSuccess(
                            `Validated ${file} (Service: ${serviceName})`,
                        );
                    } else {
                ...
                logWarning(
                    `No strictly defined schema for service: ${serviceName}. Skipping validation.`,
                );
            }
        }
    }
