...
class MockExecutionClient {
    ...
    async sendSignal(payload) {
        const signature = crypto
            ...
            .digest("hex");
        console.log(`Sending Signal to ${this.baseUrl} [Signature: ${signature.substring(0, 8)}...]`);
        try {
            ...
            if (!response.ok) {
                ...
                console.error(`❌ Execution Server Error (${response.status}): ${text}`);
                return false;
            }
            const json = await response.json();
            console.log(`✅ Signal Response:`, JSON.stringify(json));
            if (!json.success) {
                console.warn(`⚠️ Signal processed but returned failure: ${json.reason || json.error}`);
            }
            return true;
        }
        catch (error) {
...
async function getBrainEquity() {
    try {
        ...
        console.warn(`⚠️ Could not fetch Brain equity: ${e}`);
        return -1;
    }
}
async function main() {
    ...
    if (initialEquity === -1) {
        console.error("❌ Brain is not reachable. Please run './scripts/start-all.sh' first.");
        process.exit(1);
    }
    ...
    try {
        ...
        if (autoRes.ok) {
            ...
            console.error("❌ Failed to enable Auto-Execution:", await autoRes.text());
        }
    }
    catch (e) {
    ...
    if (finalEquity !== initialEquity) {
        console.log(`✅ SUCCESS: Brain Equity changed ($${initialEquity} -> $${finalEquity})`);
        console.log("   This confirms: Sentinel Signal -> Execution Order -> Execution Report -> Brain PnL.");
    }
    else {
        console.log("⚠️  WARNING: Brain Equity did not change.");
        console.log("   Possible reasons:");
        console.log("   - Execution did not broadcast fill (Auto-Execution disabled?)");
        console.log("   - Adapter did not return success (Mock keys used?)");
        console.log("   - Execution Report failed HMAC at Brain?");
        console.log("   Check '../../logs/execution.log' and '../../logs/brain.log' for details.");
    }
}
main().catch(console.error);
//# sourceMappingURL=simulation_suite.js.map