...
program
    ...
    .option("-c, --class <classification>", "Classification: MAJOR or SIGNIFICANT")
    .option("-s, --status <status>", "Status: DETECTED, INVESTIGATING, MITIGATED, RESOLVED")
    .option("--root-cause <cause>", "Root cause analysis")
    .option("--services <services>", "Comma-separated list of affected services")
...
function run(options) {
    ...
    try {
        ...
        const filename = `${new Date().toISOString().split("T")[0]}_${validated.incidentId}.md`;
...
*Generated by Titan Compliance Automation*
    `.trim();
        ...
        if (validated.isReportable) {
            console.warn("⚠️  This incident is classified as MAJOR and requires regulatory reporting under DORA Article 19.");
        }
    }
    catch (error) {
        if (error instanceof z.ZodError) {
            console.error("❌ Validation Failed:", JSON.stringify(error.issues || error, null, 2));
        }
        else {
...
program.parse();
//# sourceMappingURL=report_incident.js.map