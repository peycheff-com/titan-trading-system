...
const cmd = `npx eslint -c scripts/sota/complexity.config.mjs --format json "services/*/src/**/*.{ts,tsx}"`;
let output;
try {
...
console.log(`${BOLD}${"Score".padEnd(8)} | ${"Type".padEnd(12)} | ${"Location"}${RESET}`);
console.log("".padEnd(80, "-"));
metrics.slice(0, 20).forEach((m) => {
    const color = m.score > 30 ? RED : m.score > 20 ? YELLOW : CYAN;
    console.log(`${color}${m.score.toString().padEnd(8)}${RESET} | ${m.type.padEnd(12)} | ${m.file}:${m.line}`);
});
...
| Score | Type | Location |
|-------|------|----------|
${metrics.slice(0, 50).map((m) => `| ${m.score} | ${m.type} | [${m.file}:${m.line}](${m.file}#L${m.line}) |`).join("\n")}
`;
fs.writeFileSync(reportPath, mdContent);
console.log(`\n📄 Report saved to ${reportPath}`);
//# sourceMappingURL=rank_complexity.js.map