...
// Configuration
const ESLINT_CMD = 'npx eslint -c scripts/sota/immutability.config.mjs "services/*/src/**/*.{ts,tsx}" --format=json';
function runEslint() {
...
function applyBaseline(results) {
    let initialTotal = 0;
    let filesChanged = 0;
    for (const result of results) {
        ...
        for (const msg of result.messages) {
            ...
            if (!msg.ruleId.startsWith("functional/"))
                continue;
        ...
        const sortedLines = Array.from(messagesByLine.keys()).sort((a, b) => b - a);
        for (const lineNum of sortedLines) {
            const rules = Array.from(messagesByLine.get(lineNum));
            const disableComment = `// eslint-disable-next-line ${rules.join(", ")}`;
            ...
            if (targetLineIdx > 0 &&
                lines[targetLineIdx - 1].trim().startsWith("// eslint-disable-next-line")) {
        ...
        if (modified) {
            ...
            filesChanged++;
            console.log(`Baselined ${messagesByLine.size} violations in ${path.basename(result.filePath)}`);
        }
    }
    console.log(`\nOperation Complete.`);
...
console.log(`Found ${results.reduce((acc, r) => acc + r.messages.length, 0)} total violations.`);
applyBaseline(results);
//# sourceMappingURL=baseline_immutability.js.map