...
for (const service of services) {
    ...
    if (
        ...
        const files = fs.readdirSync(migrationsPath).filter((f) =>
...
try {
    const changedFiles = execSync(
        "git diff --name-only HEAD services/*/migrations",
    ).toString();
    if (changedFiles.length > 0) {
        console.error(
            "❌ Detected changes (mutations) to existing migrations:",
        );
        console.error(changedFiles);
        console.error(
            "Migrations should be immutable. Create a new migration instead of editing an existing one.",
        );
        process.exit(1);
    }
} catch (e) {
