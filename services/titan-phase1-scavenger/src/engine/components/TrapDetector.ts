...
export class TrapDetector {
    ...
    onBinanceTick(symbol: string, price: number, trades: Trade[]): void {
        ...
        this.leadLagDetector.recordPrice(
            symbol,
            "BINANCE",
            price,
            exchangeTime,
        ...
        for (const trap of traps) {
            ...
            if (elapsed >= 100) {
                const minTrades = this.config.getConfig().minTradesIn100ms ||
                    50;
                ...
                if (counter.count >= minTrades) {
                    ...
                    this.logger.info(
                        `   ⏳ PENDING CONFIRMATION: Waiting 200ms...`,
                    );
    ...
    private async checkConfirmation(
        ...
        if (!currentPrice) {
            this.logger.warn(
                `⚠️ Confirmation failed: No price data for ${symbol}`,
            );
            return;
        }
