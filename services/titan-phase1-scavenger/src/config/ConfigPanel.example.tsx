...
export function ConfigPanel({ configManager, onClose }: ConfigPanelProps) {
  ...
  const [selectedSection, setSelectedSection] = useState<'regime' | 'flow' | 'risk' | 'exchanges'>('regime');
  ...
  return (
    <Box flexDirection="column" borderStyle="double" borderColor="cyan" padding={1}>
      <Text bold color="cyan">⚙️ CONFIGURATION PANEL (F1 to close)</Text>
      
      {/* Regime Settings */}
      {selectedSection === 'regime' && (
        <Box flexDirection="column" marginTop={1}>
          <Text bold color="yellow">📊 Regime Settings</Text>
      ...
      {selectedSection === 'flow' && (
        <Box flexDirection="column" marginTop={1}>
          <Text bold color="yellow">🌊 Flow Settings</Text>
      ...
      {selectedSection === 'exchanges' && (
        <Box flexDirection="column" marginTop={1}>
          <Text bold color="yellow">🔄 Exchange Settings</Text>
