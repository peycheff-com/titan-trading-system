...
export class ConfigManager extends EventEmitter {
  ...
  private getConfigDirectory(): string {
    const baseDir = process.env.TITAN_CONFIG_DIR || path.join(os.homedir(), '.titan');
  ...
  private loadPhaseConfig(): TrapConfig {
    try {
      ...
      console.warn(`⚠️ Failed to load phase config: ${error instanceof Error ? error.message : 'Unknown error'}`);
  ...
  private loadBrainOverrides(): BrainOverrideConfig {
    try {
      ...
      console.warn(`⚠️ Failed to load Brain overrides: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
   ...
   */
  private validateAndMergeWithDefaults(loadedConfig: Partial<TrapConfig>): TrapConfig {
   ...
   */
  private validateBrainOverrides(overrides: Partial<BrainOverrideConfig>): BrainOverrideConfig {
    ...
    if (overrides.maxGlobalLeverage !== undefined) {
      if (overrides.maxGlobalLeverage >= 1 && overrides.maxGlobalLeverage <= 100) {
        validated.maxGlobalLeverage = overrides.maxGlobalLeverage;
      }
    }
    
    if (overrides.maxGlobalDrawdown !== undefined) {
      if (overrides.maxGlobalDrawdown >= 0.01 && overrides.maxGlobalDrawdown <= 1.0) {
    ...
    if (overrides.phase1) {
      ...
      if (overrides.phase1.maxLeverage !== undefined) {
        if (overrides.phase1.maxLeverage >= 1 && overrides.phase1.maxLeverage <= 100) {
          validated.phase1.maxLeverage = overrides.phase1.maxLeverage;
        }
      }
      
      if (overrides.phase1.maxPositionSize !== undefined) {
        if (overrides.phase1.maxPositionSize >= 0.01 && overrides.phase1.maxPositionSize <= 1.0) {
          validated.phase1.maxPositionSize = overrides.phase1.maxPositionSize;
        }
      }
      
      if (overrides.phase1.riskMultiplier !== undefined) {
        if (overrides.phase1.riskMultiplier >= 0 && overrides.phase1.riskMultiplier <= 2.0) {
  ...
  private mergeConfigurations(): MergedConfig {
    ...
    if (this.brainOverrides.phase1) {
      ...
      if (this.brainOverrides.phase1.riskMultiplier !== undefined) {
        merged.effective.riskMultiplier = this.brainOverrides.phase1.riskMultiplier;
      }
    }
  ...
  private setupHotReload(): void {
    try {
      ...
      console.warn(`⚠️ Failed to setup hot-reload: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  private handlePhaseConfigChange(): void {
    try {
      ...
      console.error(`❌ Failed to reload phase config: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  private handleBrainOverrideChange(): void {
    try {
      ...
      console.error(`❌ Failed to reload Brain overrides: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  savePhaseConfig(newConfig: TrapConfig): void {
    try {
      ...
      console.error(`❌ Failed to save phase config: ${error instanceof Error ? error.message : 'Unknown error'}`);
      throw error;
    }
  }
  ...
  updateBrainOverrides(overrides: Partial<BrainOverrideConfig>): void {
    try {
      ...
      console.error(`❌ Failed to update Brain overrides: ${error instanceof Error ? error.message : 'Unknown error'}`);
      throw error;
    }
  }
  ...
  getEffectiveConfig(): TrapConfig & { effective: MergedConfig['effective'] } {
    return {
      ...
      maxPositionSizePercent: this.mergedConfig.effective.maxPositionSizePercent,
  ...
  getConfigStatus(): {
    ...
    return {
      ...
      hotReloadEnabled: this.configWatcher !== null || this.brainWatcher !== null,
  ...
  validateConfig(config: TrapConfig): string[] {
    ...
    if (config.liquidationConfidence < 0 || config.liquidationConfidence > 100) {
    ...
    if (config.extremeVelocityThreshold < 0 || config.extremeVelocityThreshold > 0.1) {
      errors.push('extremeVelocityThreshold must be between 0 and 0.1 (10%)');
    }
    
    if (config.moderateVelocityThreshold < 0 || config.moderateVelocityThreshold > 0.05) {
      errors.push('moderateVelocityThreshold must be between 0 and 0.05 (5%)');
    }
    
    if (config.aggressiveLimitMarkup < 0 || config.aggressiveLimitMarkup > 0.01) {
    ...
    if (config.maxPositionSizePercent < 0.1 || config.maxPositionSizePercent > 1.0) {
      errors.push('maxPositionSizePercent must be between 0.1 (10%) and 1.0 (100%)');
    }
    ...
    if (!config.exchanges.bybit.enabled && !config.exchanges.mexc.enabled) {
      errors.push('At least one execution exchange (Bybit or MEXC) must be enabled');
    }
