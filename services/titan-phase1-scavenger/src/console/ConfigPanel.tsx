...
export function ConfigPanel({ config, onSave, onCancel }: ConfigPanelProps) {
  ...
  const [activeSection, setActiveSection] = useState<
    'trap' | 'volume' | 'execution' | 'risk' | 'exchanges'
  >('trap');
  ...
  return (
    <Box flexDirection="column" borderStyle="double" borderColor="cyan" padding={1}>
      ...
      {/* Section Tabs */}
      <Box marginBottom={1}>
        <Text color={activeSection === 'trap' ? 'cyan' : 'gray'}>[1] Trap Params</Text>
        <Text> </Text>
        <Text color={activeSection === 'volume' ? 'cyan' : 'gray'}>[2] Volume</Text>
        <Text> </Text>
        <Text color={activeSection === 'execution' ? 'cyan' : 'gray'}>[3] Execution</Text>
        <Text> </Text>
        <Text color={activeSection === 'risk' ? 'cyan' : 'gray'}>[4] Risk</Text>
        <Text> </Text>
        <Text color={activeSection === 'exchanges' ? 'cyan' : 'gray'}>[5] Exchanges</Text>
      </Box>
  ...
  return (
    <Box flexDirection="column">
      ...
      <Box marginTop={1}>
        ...
        <Text color="yellow">{(config.targetPercent / config.stopLossPercent).toFixed(1)}:1</Text>
      </Box>
    </Box>
  );
}
