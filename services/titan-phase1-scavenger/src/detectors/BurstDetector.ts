...
export class BurstDetector extends EventEmitter {
    ...
    private volScaler: VolatilityScaler;

    // Configuration
    private hawkesThreshold: number;
    private tailZScoreThreshold: number;

    // State
    private lastPrice: number = 0;
    private recentIntensities: number[] = [];
    ...
    constructor(
        ...
        hawkesThreshold: number = 20.0,
        tailZScoreThreshold: number = 4.0,
    ) {
        super();
     ...
     * Process a new trade
     */
    public processTrade(trade: Trade, currentVolatility: number) {
        ...
        if (this.lastPrice > 0 && currentVolatility > 0) {
            ...
            const zScore = PowerLawCalculators.calculateVolatilityZScore(
                absMove,
                currentVolatility,
            );
