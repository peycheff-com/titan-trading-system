 ...
 */
import { CredentialManager, } from "../../src/config/CredentialManager";
...
describe("CredentialManager", () => {
    ...
    describe("Constructor and Initialization", () => {
        ...
        it("should set correct credentials path", () => {
            const expectedPath = path.join(os.homedir(), ".titan-scanner", "secrets.enc");
        ...
        it("should throw error if credentials file does not exist", () => {
            ...
            expect(() => {
                try {
                    ...
                    // Restore
                    if (oldBinance)
                        process.env.BINANCE_API_KEY = oldBinance;
                }
    ...
    describe("Round-trip encryption/decryption", () => {
        it("should correctly encrypt and decrypt credentials", () => {
            ...
            const loadedCredentials = credentialManager.loadCredentials();
            expect(loadedCredentials).toEqual(sampleCredentials);
            expect(loadedCredentials.binance.apiKey).toBe(sampleCredentials.binance.apiKey);
            expect(loadedCredentials.binance.apiSecret).toBe(sampleCredentials.binance.apiSecret);
            expect(loadedCredentials.bybit.apiKey).toBe(sampleCredentials.bybit.apiKey);
            expect(loadedCredentials.bybit.apiSecret).toBe(sampleCredentials.bybit.apiSecret);
            expect(loadedCredentials.mexc.apiKey).toBe(sampleCredentials.mexc.apiKey);
            expect(loadedCredentials.mexc.apiSecret).toBe(sampleCredentials.mexc.apiSecret);
        });
    ...
    describe("updateExchangeCredentials()", () => {
        it("should update Binance credentials", () => {
            credentialManager.saveCredentials(sampleCredentials);
            credentialManager.updateExchangeCredentials("binance", "new-binance-key", "new-binance-secret");
            const loadedCredentials = credentialManager.loadCredentials();
        ...
        it("should update Bybit credentials", () => {
            credentialManager.saveCredentials(sampleCredentials);
            credentialManager.updateExchangeCredentials("bybit", "new-bybit-key", "new-bybit-secret");
            const loadedCredentials = credentialManager.loadCredentials();
        ...
        it("should update MEXC credentials", () => {
            credentialManager.saveCredentials(sampleCredentials);
            credentialManager.updateExchangeCredentials("mexc", "new-mexc-key", "new-mexc-secret");
            const loadedCredentials = credentialManager.loadCredentials();
        ...
        it("should create new credentials if none exist", () => {
            ...
            credentialManager.updateExchangeCredentials("binance", "first-binance-key", "first-binance-secret", true);
            // Add Bybit credentials to satisfy validation
            credentialManager.updateExchangeCredentials("bybit", "first-bybit-key", "first-bybit-secret", true);
            expect(credentialManager.credentialsExist()).toBe(true);
            const loadedCredentials = credentialManager.loadCredentials();
        ...
        it("should trim whitespace from credentials", () => {
            ...
            credentialManager.updateExchangeCredentials("binance", "  key-with-spaces  ", "  secret-with-spaces  ");
            const loadedCredentials = credentialManager.loadCredentials();
...
//# sourceMappingURL=CredentialManager.test.js.map