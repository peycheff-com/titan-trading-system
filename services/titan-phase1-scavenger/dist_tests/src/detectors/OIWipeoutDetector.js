...
export class OIWipeoutDetector {
    ...
    async detectWipeout(symbol) {
        ...
        try {
            ...
            console.log(`   Target: ${targetPrice.toFixed(2)} (+${((targetPrice / currentPrice - 1) * 100).toFixed(1)}%)`);
            return {
                symbol,
            ...
            if (error && (error.message || "").includes("403")) {
                if (!this.isGeoBlocked) {
                    console.warn(`⛔ Geo-blocking detected for ${symbol} (HTTP 403). Disabling OIWipeoutDetector.`);
    ...
    recordOI(symbol, oi) {
        ...
        this.oiHistory.set(symbol, history.filter((h) => h.timestamp > cutoff));
    }
    /**
...
//# sourceMappingURL=OIWipeoutDetector.js.map