...
export class UltimateBulgariaProtocol {
    bybitClient; // Will be null when using titan-execution service
    binanceClient;
    oiDetector;
    ofiCalculator;
    constructor(bybitClient, // Can be null when using titan-execution service
    binanceClient, oiDetector) {
    ...
    async scan() {
        ...
        for (const symbol of crashSymbols) {
            ...
            if (ofiScore <= 0) {
                console.log(`   OFI Check Failed: ${ofiScore.toFixed(4)} (Buying Pressure Low)`);
                continue;
            }
            console.log(`   OFI Confirmed: ${ofiScore.toFixed(4)} (Buying Pressure Detected)`);
            // 3. Set Leader-Follower trap on Binance Spot
    ...
    metrics = new Map();
    updatePowerLawMetrics(symbol, data) {
        ...
        for (const symbol of symbols) {
            try {
                ...
                if (drop > dropThreshold && btcDrop < 0.005) {
                    crashes.push(symbol);
                    console.log(`💀 Idiosyncratic crash detected: ${symbol} (-${(drop * 100).toFixed(1)}%) vs BTC (-${(btcDrop * 100).toFixed(1)}%) | Regime: ${metric ? metric.alpha.toFixed(2) : "N/A"}`);
                }
            }
            catch (error) {
     ...
     */
    async sampleOFI(symbol) {
        this.ofiCalculator.reset();
        // 5 samples * 200ms = 1 second window
        for (let i = 0; i < 5; i++) {
            try {
                const ticker = await this.bybitClient.getTicker(symbol);
                this.ofiCalculator.update(parseFloat(ticker.bid1Price), parseFloat(ticker.bid1Size), parseFloat(ticker.ask1Price), parseFloat(ticker.ask1Size));
...
//# sourceMappingURL=UltimateBulgariaProtocol.js.map