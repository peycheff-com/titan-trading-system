export class PredictionMarketDetector {
    ...
    async scan() {
        ...
        for (const market of markets) {
            ...
            if (spike > 0.15) { // 15% probability jump
                // Check Spot Lag
                ...
                if (distanceToTarget > 0 && distanceToTarget < 0.05) {
                    console.log(`🔮 ORACLE SPIKE: ${market.question} (Prob: ${(yesPrice * 100).toFixed(0)}%, +${(spike * 100).toFixed(0)}%)`);
                    tripwires.push({
                        symbol: "BTCUSDT",
    ...
    recordHistory(id, price) {
        ...
        this.history.set(id, arr.filter((x) => x.time > cutoff));
    }
    detectSpike(id, currentPrice) {
        const arr = this.history.get(id);
        if (!arr || arr.length < 2)
            return 0;
...
//# sourceMappingURL=PredictionMarketDetector.js.map