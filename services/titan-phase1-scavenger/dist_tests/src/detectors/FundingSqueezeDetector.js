...
export class FundingSqueezeDetector {
    ...
    cvdCalculator;
    isGeoBlocked = false;
    constructor(bybitClient, cvdCalculator) {
    ...
    async detectSqueeze(symbol) {
        ...
        try {
            ...
            console.log(`   Higher Low: YES (${recentLows[0].toFixed(2)} → ${recentLows[1].toFixed(2)} → ${recentLows[2].toFixed(2)})`);
            console.log(`   CVD Rising: YES (${previousCVD.toFixed(0)} → ${cvd.toFixed(0)})`);
            console.log(`   Target: ${liquidationTarget.toFixed(2)} (+${((liquidationTarget / currentPrice - 1) * 100).toFixed(1)}%)`);
            return {
                symbol,
            ...
            if (error && (error.message || "").includes("403")) {
                if (!this.isGeoBlocked) {
                    console.warn(`⛔ Geo-blocking detected for ${symbol} (HTTP 403). Disabling FundingSqueezeDetector.`);
...
//# sourceMappingURL=FundingSqueezeDetector.js.map