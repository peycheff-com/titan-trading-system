 ...
 */
import { ExecutionClient } from "@titan/shared";
...
export class TitanTrap {
    // Trap storage
    trapMap = new Map();
    volumeCounters = new Map();
    latestPrices = new Map(); // Real-time price cache for confirmation checks
    // Cached equity (updated every 5 seconds in background)
    cachedEquity = 0;
    equityUpdateInterval;
    // Pre-computation interval
    preComputationInterval;
    // Memory monitoring interval
    memoryMonitorInterval;
    // Client references (injected via constructor)
    binanceClient; // TODO: Type as BinanceSpotClient
    bybitClient; // TODO: Type as BybitPerpsClient
    executionClient; // Execution client for NATS signals
    logger; // TODO: Type as Logger
    config; // TODO: Type as ConfigManager
    eventEmitter;
    // Calculators and detectors (injected via constructor)
    tripwireCalculators; // TODO: Type as TripwireCalculators
    velocityCalculator; // TODO: Type as VelocityCalculator
    positionSizeCalculator; // TODO: Type as PositionSizeCalculator
    oiDetector; // TODO: Type as OIWipeoutDetector
    fundingDetector; // TODO: Type as FundingSqueezeDetector
    basisDetector; // TODO: Type as BasisArbDetector
    ultimateProtocol; // TODO: Type as UltimateBulgariaProtocol
    volatilityScaler;
    cvdCalculator;
    leadLagDetector;
    predictionDetector;
    ...
    constructor(dependencies) {
        ...
        this.predictionDetector = new PredictionMarketDetector(polymarket, this.binanceClient);
    ...
    setupExecutionEventListeners() {
        ...
        this.executionClient.on("reconnecting", (attempt) => {
        ...
        this.executionClient.on("message", (message) => {
            ...
            if (message.type === "status_update") {
                ...
                if (this.config && typeof this.config.updatePhaseConfig === "function") {
                    try {
                    ...
                    console.warn("⚠️ ConfigManager not available or invalid for IPC update");
                }
            }
        });
    }
    ...
    async start() {
        ...
        try {
            ...
            console.warn("⚠️ Failed to connect to Execution Service:", error instanceof Error ? error.message : "Unknown error");
            // Emit warning event
    ...
    async stop() {
        ...
        try {
            ...
            console.warn("⚠️ Error disconnecting from Execution Client:", error instanceof Error ? error.message : "Unknown error");
        }
     ...
     */
    async checkConfirmation(symbol, trap, microCVD, burstVolume) {
        ...
        if (isHolding) {
            console.log(`✅ CONFIRMATION PASSED: ${symbol} holding at ${currentPrice.toFixed(2)} vs Trigger ${trap.triggerPrice.toFixed(2)}`);
            await this.fire(trap, microCVD, burstVolume);
        }
        else {
            console.warn(`❌ CONFIRMATION FAILED (WICK): ${symbol} reverted to ${currentPrice.toFixed(2)} (Trigger: ${trap.triggerPrice.toFixed(2)})`);
    ...
    async updateTrapMap() {
        ...
        try {
            ...
            // 2. Calculate tripwires for each symbol
            const scoredSymbols = await Promise.all(symbols.map(async (symbol) => {
                try {
                    // Fetch OHLCV data
                    ...
                    const sma20 = TripwireCalculators.calcSMA(new Float64Array(ohlcv.map((b) => b.close)), 20);
                    ...
                    console.error(`   ⚠️ Failed to calculate traps for ${symbol}:`, error);
                    return {
                        symbol,
                        trapQuality: 0,
                        traps: [],
            ...
            if (this.bybitClient &&
                ...
                this.bybitClient.subscribeTicker(symbolList, this.onBybitTicker.bind(this));
                console.log(`   ✅ Subscribed to Bybit Tickers for Lead/Lag detection`);
            }
            const duration = Date.now() - startTime;
            // Warn if pre-computation exceeded 60 seconds
            if (duration > 60000) {
                console.warn(`⚠️ COMPUTATION_SLOW: Pre-computation exceeded 60s: ${duration}ms`);
            }
     ...
     */
    calculateTrapQuality(ohlcv, traps) {
        if (traps.length === 0)
            return 0;
    ...
    logTrapSummary() {
        ...
        for (const [symbol, traps] of this.trapMap.entries()) {
            ...
            for (const trap of traps) {
                console.log(`      - ${trap.trapType} @ ${trap.triggerPrice.toFixed(2)} (${trap.direction}, ${trap.confidence}% conf)`);
            }
        }
        console.log("");
    }
     ...
     */
    onBinanceTick(symbol, price, trades) {
        ...
        for (const trap of traps) {
            ...
            if (elapsed >= 100) {
                ...
                if (counter.count >= minTrades) {
                    ...
                    console.log(`⚡ TRAP SPRUNG: ${symbol} at ${price.toFixed(2)} (${counter.count} trades, CVD: ${microCVD.toFixed(4)})`);
     ...
     */
    onBybitTicker(symbol, price, timestamp) {
     ...
     */
    async fire(trap, microCVD, burstVolume) {
        let signalId;
        try {
            ...
            if (microCVD !== undefined && burstVolume !== undefined && burstVolume > 0) {
                ...
                if (!isCVDAligned) {
                    ...
                    console.warn(`🛑 MICRO-CVD VETO: Volume flow opposes trap. Direction: ${trap.direction}, CVD: ${microCVD.toFixed(4)}`);
                    return;
                }
                ...
                if (directionalRatio < 0.3) {
                    console.warn(`🛑 BURST QUALITY VETO: Low directional conviction. Ratio: ${directionalRatio.toFixed(2)} < 0.3. (CVD: ${microCVD.toFixed(4)} / Vol: ${burstVolume.toFixed(4)})`);
                    return;
                }
                console.log(`✅ MICRO-CVD CONFIRMED: ${microCVD.toFixed(4)} aligns with ${trap.direction} (Quality: ${directionalRatio.toFixed(2)})`);
            }
            ...
            if (!isCounterFlow) {
                console.warn(`⚠️ MACRO CVD INFO: Trend following detected (CVD: ${cvd}).`);
            }
            else {
            ...
            if (acceleration > 0) {
                console.warn(`🛑 KNIFE-CATCH VETO: Price is accelerating (${acceleration.toFixed(4)}). Waiting for deceleration.`);
                return; // VETO EXECUTION
            }
            console.log(`✅ ACCELERATION CHECK: Safe (Acc: ${acceleration.toFixed(4)})`);
            // --- ALPHA LOGIC: TREND FILTER (ADX) ---
            if (trap.adx && trap.adx > 25) {
                const isFadingTrend = (trap.direction === "LONG" && trap.trend === "DOWN") ||
                    (trap.direction === "SHORT" && trap.trend === "UP");
                if (isFadingTrend) {
                    console.warn(`🛑 TREND VETO: Strong Trend (ADX: ${trap.adx.toFixed(2)}) is against us. Aborting fade.`);
                    return; // VETO EXECUTION
                }
            }
            ...
            if (trap.volatilityMetrics?.atr) {
                ...
                if (trap.volatilityMetrics.regime === "HIGH_VOL") {
                    ...
                    console.log(`   🌊 High Volatility Regime: Scaling velocity thresholds x1.5`);
                }
                else if (trap.volatilityMetrics.regime === "LOW_VOL") {
                    extremeVelocity *= 0.8;
                    moderateVelocity *= 0.8;
                    console.log(`   🧊 Low Volatility Regime: Scaling velocity thresholds x0.8`);
                }
            }
            ...
            if (velocity > extremeVelocity) {
                ...
                orderType = "MARKET";
                console.log(`   🚀 Using MARKET order (velocity: ${(velocity * 100).toFixed(2)}% > ${extremeVelocity * 100}%)`);
            }
                ...
                console.log(`   ⚡ Using AGGRESSIVE LIMIT at ${limitPrice.toFixed(2)} (velocity: ${(velocity * 100).toFixed(2)}%)`);
            }
            else {
                ...
                console.log(`   📍 Using LIMIT at ${limitPrice.toFixed(2)} (velocity: ${(velocity * 100).toFixed(2)}%)`);
            }
            ...
            console.log(`   💰 Position size: ${positionSize.toFixed(4)} (Equity: $${this.cachedEquity.toFixed(2)})`);
            ...
            if (volMultiplier !== 1) {
                console.log(`   📉 Volatility Adjustment: Size scaled by ${volMultiplier.toFixed(2)}x -> ${adjustedPositionSize.toFixed(4)}`);
            }
            ...
            if (ghostMode) {
                console.log(`👻 GHOST MODE ACTIVE: Skipping IPC execution for ${trap.symbol}`);
                console.log(`👻 Virtual Trade: ${trap.direction} ${trap.symbol} @ ${limitPrice || bybitPrice} (Size: ${positionSize})`);
            ...
            try {
                ...
                if (this.isTrapStillValid(trap)) {
                    ...
                    const confirmResult = await this.executionClient.sendConfirm(signalId);
                    ...
                    console.log(`   ✅ CONFIRM acknowledged: executed=${confirmResult.executed}, fill_price=${confirmResult.fill_price} (${confirmLatency}ms, total: ${totalLatency}ms)`);
                    ...
                    console.log(`   ✅ ABORT acknowledged: aborted=${abortResult.aborted}`);
                ...
                const errorMessage = ipcError instanceof Error
                    ? ipcError.message
                    : "Unknown IPC error";
                console.warn(`⚠️ Fast Path IPC failed (${errorMessage}), falling back to HTTP POST`);
                ...
                try {
                    ...
                    throw new Error(`Both IPC and HTTP execution failed. IPC: ${errorMessage}, HTTP: ${httpError instanceof Error ? httpError.message : "Unknown error"}`);
                }
            }
        }
        catch (error) {
            ...
            if (signalId) {
                try {
                    ...
                    console.log(`   ✅ Sent ABORT for failed execution: signal_id=${signalId}`);
                }
                catch (abortError) {
    ...
    startMemoryMonitoring() {
        this.memoryMonitorInterval = setInterval(() => {
            ...
            if (Math.random() < 0.1) {
                console.log(`📊 Memory: Heap ${heapUsedMB.toFixed(1)}MB / ${heapTotalMB.toFixed(1)}MB, RSS ${rssMB.toFixed(1)}MB`);
            }
            // Emit warning if heap usage exceeds 150MB
            if (heapUsedMB > 150) {
                console.warn(`⚠️ RESOURCE_WARNING: Memory usage ${heapUsedMB.toFixed(2)}MB exceeds 150MB threshold`);
    ...
    isTrapStillValid(trap) {
        try {
            ...
            if (priceDistance > maxPriceDistance) {
                console.log(`   ⚠️ Trap validation failed: price moved ${(priceDistance * 100).toFixed(3)}% from trigger (max ${(maxPriceDistance * 100).toFixed(1)}%)`);
                return false;
            }
            ...
            if (timeSinceVolumeStart > 200) {
                console.log(`   ⚠️ Trap validation failed: volume activity stale (${timeSinceVolumeStart}ms old)`);
                return false;
            }
            console.log(`   ✅ Trap validation passed: price=${currentPrice.toFixed(2)}, distance=${(priceDistance * 100).toFixed(3)}%, volume=${volumeCounter.count} trades`);
            return true;
        }
        catch (error) {
    ...
    handleFailure(symbol) {
        ...
        if (currentFailures >= 3) {
            console.warn(`   ⛔ BLACKLISTING ${symbol} for 5 minutes (Too many failures)`);
...
//# sourceMappingURL=TitanTrap.js.map