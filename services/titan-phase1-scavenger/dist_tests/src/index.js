...
class TitanScavengerApp {
    ...
    binanceClient;
    bybitClient;
    natsClient; // New NATS Client for Metrics
    // ... existing props
    async initialize() {
        ...
        try {
            ...
            this.ultimateProtocol = new UltimateBulgariaProtocol(this.bybitClient, this.binanceClient, this.oiDetector);
            console.log('✅ Structural flaw detectors initialized');
            // Subscribe to Power Law Metrics after protocol is initialized
            await this.natsClient.subscribeToPowerLawMetrics((symbol, data) => {
                // Forward to UltimateBulgariaProtocol
                this.ultimateProtocol.updatePowerLawMetrics(symbol, data);
                // Optional: Log significant alpha shifts?
                // this.logger.debug(`Metrics updated for ${symbol}`, data);
            });
...
class TitanScavengerApp {
    ...
    sensorMonitorInterval;
    binanceTickCounter = 0;
    lastTickTime = Date.now();
    lastTrapSprungTime = null;
    // Headless mode flag
    headless;
    // Health server port
    healthPort;
    constructor(headless = false, healthPort = 8081) {
    ...
    async initialize() {
        ...
        try {
            ...
            this.bybitClient = new BybitPerpsClient(credentials.bybit.apiKey, credentials.bybit.apiSecret);
    ...
    setupEventListeners() {
        ...
        this.eventEmitter.on('TRAP_MAP_UPDATED', (data) => {
        ...
        // TRAP_SPRUNG event
        this.eventEmitter.on('TRAP_SPRUNG', (data) => {
        ...
        this.eventEmitter.on('EXECUTION_COMPLETE', (data) => {
     ...
     */
    updateSensorStatus(field, value) {
     ...
     */
    renderDashboard() {
        this.inkInstance = render(_jsx(TrapMonitor, { trapMap: this.state.trapMap, sensorStatus: this.state.sensorStatus, liveFeed: this.state.liveFeed, equity: this.state.equity, pnlPct: this.state.pnlPct }));
    }
    /**
    ...
    rerender() {
        ...
        if (this.headless)
            return;
        if (this.inkInstance) {
            this.inkInstance.rerender(_jsx(TrapMonitor, { trapMap: this.state.trapMap, sensorStatus: this.state.sensorStatus, liveFeed: this.state.liveFeed, equity: this.state.equity, pnlPct: this.state.pnlPct }));
        }
    }
    /**
