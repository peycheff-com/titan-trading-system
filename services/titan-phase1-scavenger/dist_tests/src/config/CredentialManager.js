...
export class CredentialManager {
    ...
    getMasterPassword() {
        ...
        if (!password) {
            throw new Error('TITAN_MASTER_PASSWORD environment variable not set. ' +
    ...
    saveCredentials(credentials) {
        try {
            ...
            fs.writeFileSync(this.credentialsPath, JSON.stringify(encryptedData, null, 2), { mode: 0o600 } // Read/write for owner only
            );
    ...
    loadCredentials() {
        try {
            ...
            if (encryptedData.version !== this.FORMAT_VERSION) {
                throw new Error(`Unsupported credentials format version: ${encryptedData.version}. ` +
    ...
    loadCredentialsFromEnv() {
        ...
        if (!binanceApiKey || !binanceApiSecret) {
            throw new Error('Missing Binance credentials in environment variables. ' +
                'Please set BINANCE_API_KEY and BINANCE_API_SECRET.');
        }
        if (!bybitApiKey || !bybitApiSecret) {
            throw new Error('Missing Bybit credentials in environment variables. ' +
     ...
     */
    updateExchangeCredentials(exchange, apiKey, apiSecret, skipValidation = false) {
