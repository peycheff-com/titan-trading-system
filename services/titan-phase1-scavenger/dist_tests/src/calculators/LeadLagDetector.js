...
export class LeadLagDetector {
    ...
    bybitPrices = new Map();
    windowSize = 60000; // 60 seconds rolling window
    bucketSize = 100; // 100ms buckets
    lastCalculation = new Map();
    calculationInterval = 1000; // Recalculate every 1 second
    // Default leader is BINANCE for all symbols until proven otherwise
    currentLeader = new Map();
    correlation = new Map();
    constructor() { }
    /**
     * Record a price update from an exchange
     */
    recordPrice(symbol, source, price, timestamp) {
    ...
    cleanup(symbol, currentTimestamp) {
    ...
    calculateLeadLag(symbol) {
        const binanceMap = this.binancePrices.get(symbol);
        const bybitMap = this.bybitPrices.get(symbol);
        if (!binanceMap || !bybitMap)
            return;
        // Get common timestamps (or interpolated)
        const now = Date.now();
        const start = Math.floor((now - 10000) / this.bucketSize) *
            this.bucketSize; // Look at last 10 seconds for HFT lead
        ...
        if (rPlus > rMinus && rPlus > r0) {
            if (this.currentLeader.get(symbol) !== "BINANCE") {
                console.log(`📡 Lead/Lag Flip [${symbol}]: BINANCE is leading (R+=${rPlus.toFixed(3)} vs R-=${rMinus.toFixed(3)})`);
                this.currentLeader.set(symbol, "BINANCE");
            }
        }
        else if (rMinus > rPlus && rMinus > r0) {
            if (this.currentLeader.get(symbol) !== "BYBIT") {
                console.log(`📡 Lead/Lag Flip [${symbol}]: BYBIT is leading (R-=${rMinus.toFixed(3)} vs R+=${rPlus.toFixed(3)})`);
                this.currentLeader.set(symbol, "BYBIT");
            }
        }
    }
    findNearest(map, target) {
        if (map.has(target))
    ...
    correlationCoefficient(x, y, lag) {
        // Apply lag
        let x_s, y_s;
        if (lag === 0) {
            x_s = x;
...
//# sourceMappingURL=LeadLagDetector.js.map