...
import { BybitPerpsClient, OrderParams } from '../../src/exchanges/BybitPerpsClient';
import { MEXCPerpsClient, MEXCOrderParams } from '../../src/exchanges/MEXCPerpsClient';
import { ExchangeGateway, Tripwire } from '../../src/exchanges/ExchangeGateway';
...
describe('Exchange Clients Integration Tests', () => {
  ...
  describe('MEXCPerpsClient', () => {
    ...
    it('should place order with MEXC-specific format', async () => {
      ...
      // Act
      const orderParams: MEXCOrderParams = {
        symbol: 'BTCUSDT',
        side: 'Buy',
        type: 'Market',
        qty: 0.001,
    ...
    it('should respect rate limiting (10 req/s)', async () => {
      ...
      const promises = Array.from({ length: 15 }, (_, i) =>
        client.setLeverage(`SYMBOL${i}`, 10)
      );
  ...
  describe('ExchangeGateway', () => {
    ...
    let mockBybitClient: BybitPerpsClient;
    let mockMexcClient: MEXCPerpsClient;
