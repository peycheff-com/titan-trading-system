...
import { VolatilityScaler } from "../src/calculators/VolatilityScaler";
import { FastPathClient } from "../src/ipc/FastPathClient";
...
describe("TitanTrap Alpha Logic", () => {
    ...
    let mockTripwireCalculators: any;
    let mockVelocityCalculator: any;
    let mockPositionSizeCalculator: any;
    let mockCvdCalculator: any; // Add mock for CVD calculator
    ...
    beforeEach(() => {
        ...
        (trap as any).cvdCalculator = mockCvdCalculator; // Force inject
        (trap as any).velocityCalculator = mockVelocityCalculator;
        (trap as any).tripwireCalculators = mockTripwireCalculators;

        // Mock FastPathClient methods
        (trap as any).fastPathClient.isConnected = jest.fn().mockReturnValue(
            true,
        );
        (trap as any).fastPathClient.sendPrepare = jest.fn().mockResolvedValue({
            prepared: true,
        });
        (trap as any).fastPathClient.sendConfirm = jest.fn().mockResolvedValue({
            executed: true,
            fill_price: 50000,
        });
        (trap as any).cachedEquity = 1000;
    });

    const createMockTrap = (overrides: any = {}) => ({
        symbol: "BTCUSDT",
        triggerPrice: 50000,
    ...
    test("BASELINE: Should fire when all conditions are safe", async () => {
        ...
        (trap as any).isTrapStillValid = jest.fn().mockReturnValue(true);
        ...
        expect((trap as any).fastPathClient.sendPrepare).toHaveBeenCalled(); // Should fire
    });
    ...
    test("GHOST MODE: Should Log Only and skip IPC", async () => {
        ...
        expect((trap as any).fastPathClient.sendPrepare).not.toHaveBeenCalled(); // Should NOT fire IPC
    });
    ...
    test("ACCELERATION VETO: Should abort if Price is Accelerating (Falling Knife)", async () => {
        ...
        expect((trap as any).fastPathClient.sendPrepare).not.toHaveBeenCalled();
    });
    ...
    test("TREND ALLOW: Should allow trade if Strong Trend is WITH the trade", async () => {
        ...
        expect((trap as any).fastPathClient.sendPrepare).toHaveBeenCalled();
    });
});
