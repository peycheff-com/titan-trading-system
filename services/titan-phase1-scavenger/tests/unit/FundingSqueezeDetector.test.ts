...
class MockBybitClient {
  private mockFundingRate: number = 0;
...
class MockCVDCalculator {
  ...
  setMockCVD(windowSeconds: number, cvd: number, offsetSeconds?: number): void {
    const key = offsetSeconds ? `${windowSeconds}_${offsetSeconds}` : `${windowSeconds}`;
    this.cvdValues.set(key, cvd);
  }

  async calcCVD(symbol: string, windowSeconds: number, offsetSeconds?: number): Promise<number> {
    const key = offsetSeconds ? `${windowSeconds}_${offsetSeconds}` : `${windowSeconds}`;
...
describe('FundingSqueezeDetector', () => {
  let detector: FundingSqueezeDetector;
  ...
  beforeEach(() => {
    ...
    detector = new FundingSqueezeDetector(mockBybitClient as any, mockCVDCalculator as any);
  });
