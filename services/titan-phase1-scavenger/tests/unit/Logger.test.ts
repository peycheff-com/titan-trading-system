...
describe('Logger', () => {
  ...
  describe('Log Rotation', () => {
    test('should rotate log when size exceeds 10MB', () => {
      ...
      const files = fs.readdirSync(testLogDir);
      const rotatedFiles = files.filter(f => f.startsWith('trades-') && f.endsWith('.jsonl'));
  ...
  describe('Log Compression', () => {
    test('should compress logs older than 30 days', async () => {
      ...
      const oldLogPath = path.join(testLogDir, 'trades-2024-01-01T00-00-00-000Z.jsonl');
  ...
  describe('queryLogs()', () => {
    test('should return all log entries', () => {
      ...
      logger.log({ timestamp: Date.now(), type: 'execution', symbol: 'ETHUSDT' });
    ...
    test('should filter log entries', () => {
      logger.log({ timestamp: Date.now(), type: 'signal', symbol: 'BTCUSDT', trapType: 'LIQUIDATION' });
      logger.log({ timestamp: Date.now(), type: 'signal', symbol: 'ETHUSDT', trapType: 'DAILY_LEVEL' });
      logger.log({ timestamp: Date.now(), type: 'execution', symbol: 'BTCUSDT' });
    ...
    test('should filter by trap type', () => {
      logger.log({ timestamp: Date.now(), type: 'signal', symbol: 'BTCUSDT', trapType: 'LIQUIDATION' });
      logger.log({ timestamp: Date.now(), type: 'signal', symbol: 'ETHUSDT', trapType: 'LIQUIDATION' });
      logger.log({ timestamp: Date.now(), type: 'signal', symbol: 'SOLUSDT', trapType: 'DAILY_LEVEL' });

      const liquidations = logger.queryLogs(entry => entry.trapType === 'LIQUIDATION');
      
      expect(liquidations.length).toBe(2);
      expect(liquidations.every(e => e.trapType === 'LIQUIDATION')).toBe(true);
    });
  });
});
