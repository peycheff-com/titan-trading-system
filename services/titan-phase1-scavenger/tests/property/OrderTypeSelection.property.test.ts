 ...
 */
function selectOrderType(velocity: number): 'MARKET' | 'AGGRESSIVE_LIMIT' | 'LIMIT' {
...
describe('Property-Based Test: Order Type Selection Determinism', () => {
  ...
  test('Property 3a: Velocity > 0.5%/s should always produce MARKET order', async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.double({ min: 0.005001, max: 0.02, noNaN: true, noDefaultInfinity: true }),
        async (velocity) => {
  ...
  test('Property 3b: Velocity 0.1-0.5%/s should always produce AGGRESSIVE_LIMIT order', async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.double({ min: 0.001001, max: 0.004999, noNaN: true, noDefaultInfinity: true }),
        async (velocity) => {
  ...
  test('Property 3c: Velocity < 0.1%/s should always produce LIMIT order', async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.double({ min: 0, max: 0.000999, noNaN: true, noDefaultInfinity: true }),
        async (velocity) => {
