...
class HealthChecker extends EventEmitter {
  ...
  private async checkServerHealth(server: BackendServer): Promise<void> {
    ...
    if (health.isHealthy !== wasHealthy) {
      ...
      this.emit('healthChange', { serverId: server.id, isHealthy: health.isHealthy });
    }
...
export class LoadBalancer extends EventEmitter {
  ...
  private selectIpHash(servers: BackendServer[], routingInfo: RoutingInfo): BackendServer {
    const clientIp = routingInfo.headers['x-forwarded-for'] || routingInfo.headers['x-real-ip'] || routingInfo.clientId;
  ...
  getHealthyServers(): BackendServer[] {
    ...
    return healthyIds.map(id => this.servers.get(id)).filter(server => server !== undefined) as BackendServer[];
  }
