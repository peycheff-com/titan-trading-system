...
export class Logger {
  ...
  static createConfigFromEnv(component: string): LoggerConfig {
    ...
    return {
      ...
      sensitiveFields: (
        process.env.LOG_SENSITIVE_FIELDS || 'password,secret,token,key,authorization'
      ).split(','),
  ...
  logTradeEntry(entry: Partial<TradeLogEntry>): void {
    ...
    try {
      const unifiedEntry: TradeLogEntry = {
        ...
        level: (entry.level as any) || (entry.type === 'error' ? 'error' : 'info'),
  ...
  private generateTradeMessage(entry: Partial<TradeLogEntry>): string {
    ...
    if (entry.type === 'close') {
      return `Close: ${entry.symbol} @ ${entry.exitPrice} (${(entry.profitPercent || 0).toFixed(
        2,
      )}%)`;
    }
    if (entry.type === 'error') {
