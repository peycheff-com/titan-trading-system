 ...
 */
export function getAIProvider(type: AIProviderType, config?: AIProviderConfig): AIProvider {
...
export function createProvider(type: AIProviderType, config?: AIProviderConfig): AIProvider {
  switch (type) {
    ...
    case 'kimi-local': {
      const localEndpoint =
        (config as KimiK2Config)?.localEndpoint ?? process.env.KIMI_LOCAL_ENDPOINT;
      if (!localEndpoint) {
        throw new Error(
          'KIMI_LOCAL_ENDPOINT environment variable or localEndpoint config required for kimi-local provider',
        );
      }
      return new KimiK2Provider({
        type: 'kimi',
    ...
    case 'gemini':
      ...
      throw new Error(
        'Gemini provider requires @titan/ai-quant/GeminiClient. Use getGeminiAdapter() instead.',
      );
 ...
 */
export function createProviderChain(config: AIProviderFactoryConfig): AIProviderChain {
...
export class AIProviderChain {
   ...
   */
  async executeWithFallback<T>(fn: (provider: AIProvider) => Promise<T>): Promise<T> {
    try {
...
export function createProviderFromEnv(): AIProvider {
  ...
  if (process.env.GEMINI_API_KEY) {
    throw new Error(
      'Gemini provider requires adapter. Set AI_PROVIDER=kimi and provide KIMI_API_KEY.',
    );
  }

  throw new Error(
    'No AI provider configured. Set AI_PROVIDER and corresponding API key environment variable.',
  );
}
