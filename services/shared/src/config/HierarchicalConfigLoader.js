...
exports.HierarchicalConfigLoader = void 0;
...
class HierarchicalConfigLoader {
    ...
    async loadBrainConfig() {
        ...
        const baseConfigPath = (0, path_1.join)(this.options.configDirectory, 'brain.config.json');
        if ((0, fs_1.existsSync)(baseConfigPath)) {
        ...
        if (this.options.enableEnvironmentFiles) {
            const envConfigPath = (0, path_1.join)(this.options.configDirectory, `brain.${this.options.environment}.config.json`);
        ...
        if (this.options.validateSchema) {
            validation = ConfigSchema_1.ConfigValidator.validateBrainConfig(config);
            if (!validation.valid) {
    ...
    async loadPhaseConfig(phase) {
        ...
        // 1. Load base configuration file
        const baseConfigPath = (0, path_1.join)(this.options.configDirectory, `${phase}.config.json`);
        if ((0, fs_1.existsSync)(baseConfigPath)) {
            const baseConfig = this.loadJsonFile(baseConfigPath);
            config = this.mergeConfigs(config, baseConfig);
            sources.push({
        ...
        // 2. Load environment-specific configuration file
        if (this.options.enableEnvironmentFiles) {
            const envConfigPath = (0, path_1.join)(this.options.configDirectory, `${phase}.${this.options.environment}.config.json`);
            if ((0, fs_1.existsSync)(envConfigPath)) {
                const envConfig = this.loadJsonFile(envConfigPath);
                config = this.mergeConfigs(config, envConfig);
                sources.push({
        ...
        let validation = { valid: true, errors: [], warnings: [] };
        if (this.options.validateSchema) {
            validation = ConfigSchema_1.ConfigValidator.validatePhaseConfig(config);
            if (!validation.valid) {
    ...
    async loadInfrastructureConfig() {
        ...
        let config = {};
        // 1. Load base configuration file
        const baseConfigPath = (0, path_1.join)(this.options.configDirectory, 'infrastructure.config.json');
        if ((0, fs_1.existsSync)(baseConfigPath)) {
            const baseConfig = this.loadJsonFile(baseConfigPath);
            config = this.mergeConfigs(config, baseConfig);
            sources.push({
        ...
        // 2. Load environment-specific configuration file
        if (this.options.enableEnvironmentFiles) {
            const envConfigPath = (0, path_1.join)(this.options.configDirectory, `infrastructure.${this.options.environment}.config.json`);
            if ((0, fs_1.existsSync)(envConfigPath)) {
                const envConfig = this.loadJsonFile(envConfigPath);
                config = this.mergeConfigs(config, envConfig);
                sources.push({
        ...
        let validation = { valid: true, errors: [], warnings: [] };
        if (this.options.validateSchema) {
            validation = ConfigSchema_1.ConfigValidator.validateInfrastructureConfig(config);
            if (!validation.valid) {
    ...
    async loadDeploymentConfig() {
        ...
        let config = {};
        // 1. Load base configuration file
        const baseConfigPath = (0, path_1.join)(this.options.configDirectory, 'deployment.config.json');
        if ((0, fs_1.existsSync)(baseConfigPath)) {
            const baseConfig = this.loadJsonFile(baseConfigPath);
            config = this.mergeConfigs(config, baseConfig);
            sources.push({
        ...
        // 2. Load environment-specific configuration file
        if (this.options.enableEnvironmentFiles) {
            const envConfigPath = (0, path_1.join)(this.options.configDirectory, `deployment.${this.options.environment}.config.json`);
            if ((0, fs_1.existsSync)(envConfigPath)) {
                const envConfig = this.loadJsonFile(envConfigPath);
                config = this.mergeConfigs(config, envConfig);
                sources.push({
        ...
        let validation = { valid: true, errors: [], warnings: [] };
        if (this.options.validateSchema) {
            validation = ConfigSchema_1.ConfigValidator.validateDeploymentConfig(config);
            if (!validation.valid) {
    ...
    async loadServiceConfig(service) {
        ...
        let config = {};
        // 1. Load base configuration file
        const baseConfigPath = (0, path_1.join)(this.options.configDirectory, `${service}.config.json`);
        if ((0, fs_1.existsSync)(baseConfigPath)) {
            const baseConfig = this.loadJsonFile(baseConfigPath);
            config = this.mergeConfigs(config, baseConfig);
            sources.push({
        ...
        // 2. Load environment-specific configuration file
        if (this.options.enableEnvironmentFiles) {
            const envConfigPath = (0, path_1.join)(this.options.configDirectory, `${service}.${this.options.environment}.config.json`);
            if ((0, fs_1.existsSync)(envConfigPath)) {
                const envConfig = this.loadJsonFile(envConfigPath);
                config = this.mergeConfigs(config, envConfig);
                sources.push({
        ...
        let validation = { valid: true, errors: [], warnings: [] };
        if (this.options.validateSchema) {
            validation = ConfigSchema_1.ConfigValidator.validateServiceConfig(service, config);
    ...
    getHierarchySummary() {
        ...
        // Add service configs
        const serviceConfigs = ConfigSchema_1.ConfigValidator.getAvailableServiceSchemas();
        configFiles.push(...serviceConfigs);
        const enabledSources = ['base-file', 'default'];
        if (this.options.enableEnvironmentFiles)
            enabledSources.push('env-file');
        if (this.options.enableEnvironmentVariables)
            enabledSources.push('environment');
        return {
            environment: this.options.environment,
            configDirectory: (0, path_1.resolve)(this.options.configDirectory),
            enabledSources,
            availableConfigs: configFiles
        };
    }
}
exports.HierarchicalConfigLoader = HierarchicalConfigLoader;
/**
