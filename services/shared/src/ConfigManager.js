...
const HierarchicalConfigLoader_1 = require("./config/HierarchicalConfigLoader");
const ConfigSchema_1 = require("./config/ConfigSchema");
const ConfigVersionHistory_1 = require("./config/ConfigVersionHistory");
...
class ConfigWatcher {
    ...
    watch(filePath, callback) {
        ...
        (0, fs_1.watchFile)(filePath, { interval: 1000 }, (curr, prev) => {
 ...
 */
class ConfigManager extends eventemitter3_1.EventEmitter {
    brainConfig = null;
    ...
    constructor(configDirectory = './config', environment) {
        ...
        this.versionHistory = (0, ConfigVersionHistory_1.getConfigVersionHistory)((0, path_1.join)(configDirectory, '.history'), 100, // Keep last 100 versions
    ...
    saveBrainConfig(config) {
        const validation = ConfigSchema_1.ConfigValidator.validateBrainConfig(config);
        if (!validation.valid) {
            throw new Error(`Invalid brain configuration: ${validation.errors.join(', ')}`);
        }
        const configPath = (0, path_1.join)(this.configDirectory, 'brain.config.json');
        try {
    ...
    savePhaseConfig(phase, config) {
        const validation = ConfigSchema_1.ConfigValidator.validatePhaseConfig(config);
        if (!validation.valid) {
            throw new Error(`Invalid ${phase} configuration: ${validation.errors.join(', ')}`);
        }
        // Validate against brain limits
        this.validateAgainstBrainLimits(phase, config);
        const configPath = (0, path_1.join)(this.configDirectory, `${phase}.config.json`);
        try {
            (0, fs_1.writeFileSync)(configPath, JSON.stringify(config, null, 2), 'utf8');
 ...
 */
function getConfigManager(configDirectory, environment) {
