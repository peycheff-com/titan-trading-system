...
export class PowerLawCalculators {
    ...
    static calculateSquareRootImpact(
        ...
        const impactDecimal = Y * dailyVolatility *
            Math.sqrt(orderSize / dailyVolume);
     ...
     */
    static calculateVolatilityZScore(
        priceMovePct: number,
     ...
     */
    static calculateHillEstimator(
        absoluteReturns: number[],
        tailPercentile: number = 0.05,
    ): number {
        ...
        const k = Math.max(
            2,
            Math.floor(sortedReturns.length * tailPercentile),
        );
