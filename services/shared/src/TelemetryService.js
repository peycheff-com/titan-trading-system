...
exports.resetTelemetryService = resetTelemetryService;
...
class LogRotationManager {
    ...
    rotateLogFile(filePath) {
        ...
        try {
            (0, fs_1.renameSync)(filePath, rotatedPath);
 ...
 */
class TelemetryService extends eventemitter3_1.EventEmitter {
    config;
    rotationManager;
    metricsCollector;
    ...
    writeToTradingLog(entry) {
        try {
            const tradingLogPath = (0, path_1.join)(this.config.logDirectory, 'trades.jsonl');
            // Check if rotation is needed
            ...
            // Write log entry
            const logLine = LogFormatter.formatFile(entry);
            (0, fs_1.appendFileSync)(tradingLogPath, logLine, 'utf8');
        }
        catch (error) {
 ...
 */
function resetTelemetryService() {
    if (telemetryServiceInstance) {
