...
import { WebSocketManager } from '../../dist/WebSocketManager';
...
describe('WebSocket Optimization Property Tests', () => {
  ...
  describe('Property 8.1: Message Batching Effectiveness', () => {
    ...
    test('should maintain message ordering within batches', () => {
      fc.assert(fc.property(
        fc.array(fc.record({
          symbol: fc.string({ minLength: 3, maxLength: 10 }).filter(s => s.trim().length >= 3).map(s => s.replace(/[^A-Z0-9]/g, 'X').toUpperCase()),
          price: fc.float({ min: Math.fround(0.01), max: Math.fround(100000), noNaN: true }),
          volume: fc.float({ min: Math.fround(0.001), max: Math.fround(1000000), noNaN: true }),
    ...
    test('should compress large messages effectively', () => {
      fc.assert(fc.property(
        ...
        (config) => {
          ...
          expect(config.messageSize).toBeGreaterThan(config.compressionThreshold);
          
          // Property: Large messages should be candidates for compression
          const shouldCompress = config.messageSize > config.compressionThreshold;
    ...
    test('should not compress small messages unnecessarily', () => {
      fc.assert(fc.property(
        ...
        (config) => {
          ...
          const shouldNotCompress = config.messageSize < config.compressionThreshold;
          expect(shouldNotCompress).toBe(true);
          
          // Property: Compression threshold should be larger than message size
          expect(config.compressionThreshold).toBeGreaterThan(config.messageSize);
  ...
  describe('Property 8.3: Delta Update Effectiveness', () => {
    ...
    test('should generate efficient delta updates for similar data', () => {
      fc.assert(fc.property(
        fc.record({
          basePrice: fc.float({ min: Math.fround(1000), max: Math.fround(50000), noNaN: true }),
          priceChanges: fc.array(fc.float({ min: Math.fround(-100), max: Math.fround(100), noNaN: true }), { minLength: 5, maxLength: 20 }),
          symbol: fc.string({ minLength: 3, maxLength: 8 }).filter(s => s.trim().length >= 3).map(s => s.replace(/[^A-Z0-9]/g, 'X').toUpperCase())
        }),
        (config) => {
      ...
      fc.assert(fc.property(
        fc.record({
          initialData: fc.record({
            price: fc.float({ min: Math.fround(1), max: Math.fround(100000), noNaN: true }),
            volume: fc.float({ min: Math.fround(0.001), max: Math.fround(1000000), noNaN: true }),
            timestamp: fc.integer({ min: 1600000000001, max: 2000000000000 })
          }),
          updates: fc.array(fc.record({
            priceChange: fc.option(fc.float({ min: Math.fround(-100), max: Math.fround(1000), noNaN: true })),
            volumeChange: fc.option(fc.float({ min: Math.fround(-1000), max: Math.fround(100000), noNaN: true })),
        ...
        (config) => {
          ...
          expect(config.initialData.timestamp).toBeGreaterThanOrEqual(1600000000001);
          ...
          for (const update of config.updates) {
            ...
            expect(currentTimestamp).toBeGreaterThan(config.initialData.timestamp);
          }
    ...
    test('should monitor connection health metrics accurately', () => {
      fc.assert(fc.property(
        fc.record({
          ...
          errorRateThreshold: fc.float({ min: Math.fround(0.01), max: Math.fround(0.1), noNaN: true })
        }),
        (config) => {
