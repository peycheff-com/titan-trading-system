        ...
        fc.property(
          ...
          fc.uniqueArray(
            fc.string({ minLength: 3, maxLength: 10 }).map((s) =>
              s.replace(/[^A-Z0-9]/g, "X").toUpperCase()
            ).filter((s) => s.length >= 3),
          ...
          (exchangeConfig, symbols, callbacksPerSymbol) => {
            ...
            expect(allSubscriptions[exchangeConfig.exchange].sort()).toEqual(
              symbols.sort(),
            );
            ...
            for (const symbol of symbols) {
              ...
              const callbacksToRemove = callbacks.slice(
                0,
                Math.floor(callbacks.length / 2),
              );

              for (const callback of callbacksToRemove) {
                testManager.unsubscribe(
                  exchangeConfig.exchange,
                  symbol,
                  callback,
                );
            ...
            for (const symbol of symbols) {
              const callbacks = subscriptionMap.get(symbol);
              const remainingCallbacks = callbacks.slice(
                Math.floor(callbacks.length / 2),
              );

              if (remainingCallbacks.length > 0) {
                expect(remainingSubscriptions[exchangeConfig.exchange])
                  .toContain(symbol);
              }
            }
    ...
    test("should handle subscription cleanup correctly", () => {
      fc.assert(
        fc.property(
          ...
          fc.uniqueArray(
            fc.string({ minLength: 3, maxLength: 8 }).map((s) =>
              s.replace(/[^A-Z0-9]/g, "X").toUpperCase()
            ).filter((s) => s.length >= 3),
          ...
          (exchangeConfig, symbols) => {
            ...
            expect(subscriptions[exchangeConfig.exchange]).toHaveLength(
              symbols.length,
            );
      ...
      fc.assert(
        fc.property(
          ...
          (exchanges) => {
            ...
            expect(Object.keys(allStatuses)).toHaveLength(
