...
import { ResourceOptimizer, MemoryStats, CPUStats, ResourceThresholds, BenchmarkResult } from '../../dist/ResourceOptimizer';
    ...
    test('should maintain memory usage within configured thresholds', () => {
      fc.assert(fc.property(
        ...
        (config) => {
          ...
          expect(config.warningThresholdPercent).toBeLessThan(config.criticalThresholdPercent);
          ...
          const totalAllocationMB = config.allocationSizeMB * config.allocationCount;
          
          // Property: Total allocation should not exceed memory limit
          if (totalAllocationMB > config.memoryLimitMB * (config.criticalThresholdPercent / 100)) {
            // This should trigger memory optimization
            expect(totalAllocationMB).toBeGreaterThan(config.memoryLimitMB * (config.criticalThresholdPercent / 100));
          }
    ...
    test('should handle memory pressure scenarios correctly', () => {
      fc.assert(fc.property(
        fc.record({
          ...
          memoryGrowthRate: fc.float({ min: Math.fround(1.1), max: Math.fround(2.0), noNaN: true }),
    ...
    test('should optimize memory usage through garbage collection', () => {
      fc.assert(fc.property(
        ...
        (config) => {
          ...
          const usedHeapBytes = heapSizeBytes * (config.usedHeapPercent / 100);
          ...
          expect(memoryStats.rss).toBeGreaterThanOrEqual(memoryStats.heapTotal);
          
          // Property: Heap usage percentage should match configuration
          const actualHeapPercent = (memoryStats.heapUsed / memoryStats.heapTotal) * 100;
    ...
    test('should maintain CPU usage within configured thresholds', () => {
      fc.assert(fc.property(
        ...
        (config) => {
          ...
          expect(config.targetCpuPercent).toBeLessThan(config.burstCpuPercent);
          ...
          if (config.workloadIntensity === 'heavy') {
            expect(expectedCpuUsage).toBeLessThanOrEqual(config.burstCpuPercent + 20); // Allow 20% tolerance
          }
    ...
    test('should handle CPU load balancing across cores', () => {
      fc.assert(fc.property(
        ...
        (config) => {
          ...
          expect(tasksPerCore * config.coreCount).toBeGreaterThanOrEqual(config.taskCount);
          ...
          if (config.loadBalancingEnabled) {
            ...
            const maxTasksPerCore = Math.ceil(config.taskCount / config.coreCount);
            const minTasksPerCore = Math.floor(config.taskCount / config.coreCount);
    ...
    test('should optimize CPU usage through throttling', () => {
      fc.assert(fc.property(
        fc.record({
          ...
          cpuSpikes: fc.array(fc.integer({ min: 80, max: 100 }), { minLength: 1, maxLength: 10 }),
        ...
        (config) => {
          ...
          expect(config.throttleThreshold).toBeGreaterThan(config.baseCpuUsage);
          ...
          const throttledSpikes = config.cpuSpikes.map(spike => {
            if (spike > config.throttleThreshold) {
              return Math.max(spike - config.throttleReduction, config.throttleThreshold);
            }
            return spike;
          });
          ...
          for (let i = 0; i < config.cpuSpikes.length; i++) {
            if (config.cpuSpikes[i] > config.throttleThreshold) {
              expect(throttledSpikes[i]).toBeLessThanOrEqual(config.cpuSpikes[i]);
            }
          }
    ...
    test('should maintain disk usage within configured limits', () => {
      fc.assert(fc.property(
        ...
        (config) => {
          ...
          const usageThresholdGB = config.diskCapacityGB * (config.usageThresholdPercent / 100);
    ...
    test('should handle disk cleanup and optimization', () => {
      fc.assert(fc.property(
        fc.record({
          ...
          compressionRatio: fc.float({ min: Math.fround(0.3), max: Math.fround(0.8), noNaN: true }),
          cleanupEfficiency: fc.float({ min: Math.fround(0.5), max: Math.fround(0.9) })
        }),
        (config) => {
          ...
          const totalUncompressedMB = config.logFileSizeMB * config.logFileCount;
          const totalCompressedMB = totalUncompressedMB * config.compressionRatio;
          const spaceFreedByCleanup = totalUncompressedMB * config.cleanupEfficiency;
          ...
          expect(spaceFreedByCleanup).toBeGreaterThan(totalUncompressedMB * 0.1); // At least 10%
      ...
      fc.assert(fc.property(
        ...
        (config) => {
          ...
          expect(config.alertThresholds.disk).toBeGreaterThanOrEqual(
            Math.min(config.alertThresholds.memory, config.alertThresholds.cpu) - 10
          );
    ...
    test('should handle alert escalation correctly', () => {
      fc.assert(fc.property(
        fc.record({
          ...
          usageIncrements: fc.array(fc.integer({ min: 5, max: 15 }), { minLength: 3, maxLength: 10 }),
    ...
    test('should provide consistent benchmark results', () => {
      fc.assert(fc.property(
        fc.record({
          ...
          operationType: fc.constantFrom('cpu_intensive', 'memory_intensive', 'io_intensive'),
        ...
        (config) => {
          ...
          expect(config.warmupIterations).toBeLessThan(config.benchmarkIterations);
          ...
          expect(['cpu_intensive', 'memory_intensive', 'io_intensive']).toContain(config.operationType);
          ...
          for (let i = 0; i < 5; i++) { // Run 5 benchmark rounds
            ...
            const memoryAfter: MemoryStats = {
              ...memoryBefore,
              heapUsed: memoryBefore.heapUsed + Math.random() * 50 * 1024 * 1024,
