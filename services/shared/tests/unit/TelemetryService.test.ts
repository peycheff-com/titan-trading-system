...
import { TelemetryService, LogLevel, SignalData, ExecutionData, MetricData } from '../../dist/TelemetryService';
...
describe('TelemetryService Unit Tests', () => {
  ...
  describe('Basic Functionality', () => {
    ...
    it('should create child loggers', () => {
      const childLogger = telemetryService.createChildLogger('test-service', 'phase1');
    ...
    it('should emit events for logs, signals, executions, and metrics', (done) => {
      ...
      telemetryService.on('log', (entry) => {
        expect(entry.service).toBe('test-service');
        checkComplete();
      });

      telemetryService.on('signal', (data) => {
        expect(data.phase).toBe('phase1');
        checkComplete();
      });

      telemetryService.on('execution', (data) => {
        expect(data.phase).toBe('phase1');
        checkComplete();
      });

      telemetryService.on('metric', (metric) => {
  ...
  describe('Error Handling', () => {
    it('should handle logging errors gracefully', () => {
      ...
      expect(() => {
        ...
        serviceWithoutFile.error('test', 'error message', new Error('Test error'));
      }).not.toThrow();
