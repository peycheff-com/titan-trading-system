...
async function main() {
  ...
  try {
    ...
    console.log(
      `[titan-opsd] Connected to NATS. Listening on ${TITAN_SUBJECTS.OPS.COMMAND}...`,
    );

    nats.subscribe(
      TITAN_SUBJECTS.OPS.COMMAND,
      async (data: unknown, subject: string) => {
        ...
        if (!parseResult.success) {
          console.error(
            "[titan-opsd] Schema validation failed",
            parseResult.error,
          );
          return;
        }
        ...
        if (!verifyOpsCommand(cmd, OPS_SECRET!)) {
          console.error(
            `[titan-opsd] Signature verification failed for cmd ${cmd.id}`,
          );
          await sendReceipt(
            nats,
            cmd,
        ...
        try {
          console.log(
            `[titan-opsd] Executing ${cmd.type} target=${cmd.target}`,
          );
          ...
          await sendReceipt(
            nats,
            cmd,
            OpsReceiptStatus.SUCCESS,
            result,
            undefined,
            duration,
          );
          console.log(
            `[titan-opsd] Command ${cmd.id} executed in ${duration}ms`,
          );
        } catch (err) {
          const duration = Date.now() - start;
