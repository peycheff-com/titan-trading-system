...
impl MexcAdapter {
    ...
    async fn request<T: serde::de::DeserializeOwned>(
        ...
        let base_resp: MexcBaseResponse<T> = serde_json::from_str(&text).map_err(|e| {
        ...
        if !base_resp.success {
            return Err(ExchangeError::Api(format!(
                "MEXC API Error {}: {}",
                base_resp.code, base_resp.message
            )));
        }
...
impl ExchangeAdapter for MexcAdapter {
    ...
    async fn place_order(&self, order: OrderRequest) -> Result<OrderResponse, ExchangeError> {
        ...
        let resp: MexcOrderResult = self
            .request(Method::POST, "/api/v1/private/order/submit", Some(payload))
            .await?;
    ...
    async fn cancel_order(
        ...
        let _resp: serde_json::Value = self
            .request(Method::POST, "/api/v1/private/order/cancel", Some(payload))
            .await?;
    ...
    async fn get_balance(&self, asset: &str) -> Result<Decimal, ExchangeError> {
        let resp: serde_json::Value = self
            .request(Method::GET, "/api/v1/private/account/assets", None)
            .await?;
