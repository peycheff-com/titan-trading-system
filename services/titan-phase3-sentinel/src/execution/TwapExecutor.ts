...
export class TwapExecutor extends EventEmitter {
  ...
  constructor(executor: IOrderExecutor, config?: TwapConfig) {
    super();
  ...
  async execute(request: TwapRequest): Promise<TwapResult> {
    ...
    try {
      ...
      for (let i = 0; i < clips.length; i++) {
        ...
        // Execute clip
        const clipResult = await this.executeClip(request.symbol, request.side, clipSize, i + 1);
        ...
        if (i < clips.length - 1) {
          const delay = this.calculateDelay(request.duration, clips.length);
  ...
  private calculateClips(request: TwapRequest): number[] {
    const avgClipSize = Math.min(request.totalSize, this.config.maxClipSize);
  ...
  private calculateDelay(totalDuration: number, numClips: number): number {
    ...
    return Math.max(this.config.minInterval, Math.min(this.config.maxInterval, avgInterval));
  }
  ...
  private async executeClip(
    ...
    const slippage = Math.abs((orderResult.avgPrice - currentPrice) / currentPrice);
