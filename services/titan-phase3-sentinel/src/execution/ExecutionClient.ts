...
export class ExecutionClient {
  ...
  constructor(
    baseUrl: string = process.env.TITAN_EXECUTION_URL || 'http://localhost:8080',
  ...
  async sendSignal(payload: SignalPayload): Promise<boolean> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error(`Execution Client Error (${response.status}): ${errText}`);
        return false;
      }
  ...
  private generateSignature(body: any): string {
    return crypto.createHmac('sha256', this.hmacSecret).update(JSON.stringify(body)).digest('hex');
  }
}
