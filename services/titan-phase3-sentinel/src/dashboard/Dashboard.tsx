...
export const Dashboard: React.FC<DashboardProps> = ({ core }) => {
    ...
    const [prices, setPrices] = useState<{ spot: number; perp: number; basis: number }>({ spot: 0, perp: 0, basis: 0 });
    ...
    return (
        <Box flexDirection="column" padding={1}>
            <Box borderStyle="double" borderColor="cyan" paddingX={1}>
                <Text bold color="cyan">TITAN SENTINEL v3.0</Text>
                <Text> | </Text>
            ...
            <Box flexDirection="column" marginTop={1}>
                <Box marginBottom={1}>
                    <Text color="grey" dimColor>TAB: {activeTab.toUpperCase()} | 'q' to quit</Text>
                </Box>
                
                {activeTab === 'monitor' && (
                    <BasisMonitor signals={signals} />
                )}
                
                {activeTab === 'inventory' && (
                    <InventoryPanel health={health} />
                )}

                {activeTab === 'logs' && (
                    <Box flexDirection="column" borderStyle="single">
                        {logs.map((log, i) => (
                            <Text key={i} color={log.includes("ERROR") ? "red" : "white"}>{log}</Text>
                        ))}
                    </Box>
                )}
            </Box>
