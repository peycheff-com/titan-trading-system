...
export const NavPanel: React.FC<NavPanelProps> = ({ health }) => {
  ...
  const utilColor =
    health.marginUtilization > 0.5 ? 'red' : health.marginUtilization > 0.3 ? 'yellow' : 'green';
      ...
      <Box flexDirection="row" gap={2}>
        <Box flexDirection="column">
          ...
          <Text bold color="green">
            ${health.nav.toFixed(2)}
          </Text>
        </Box>
        <Box flexDirection="column">
          <Text>Delta:</Text>
          <Text bold color={deltaColor}>
            ${health.delta.toFixed(2)}
          </Text>
        </Box>
        <Box flexDirection="column">
          <Text>Margin Util:</Text>
          <Text bold color={utilColor}>
            {(health.marginUtilization * 100).toFixed(1)}%
          </Text>
        </Box>
        <Box flexDirection="column">
          <Text>Status:</Text>
          <Text bold color={health.riskStatus === 'HEALTHY' ? 'green' : 'red'}>
            {health.riskStatus}
          </Text>
        </Box>
      </Box>
