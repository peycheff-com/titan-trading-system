...
async function main() {
    ...
    try {
        ...
        if (firstToken) {
            console.log(
                `Fetching orderbook for token: ${firstToken.tokenId} (${firstToken.outcomeId})`,
            );
            const book = await client.getOrderbook(firstToken.tokenId);
            console.log(
                `SUCCESS: Got Orderbook. Bids: ${book.bids.length}, Asks: ${book.asks.length}`,
            );

            if (book.bids.length > 0 && book.asks.length > 0) {
                const mid =
                    (parseFloat(book.bids[0].price) +
                        parseFloat(book.asks[0].price)) / 2;
