...
import { ConnectionStatus, GlobalCVDData, SignalValidationResponse } from '../../types';
...
export class MockGlobalLiquidity extends GlobalLiquidityAggregator {
  ...
  public updateState(timestamp: number) {
    ...
    const activeScenario = this.scenarios.find(
      s => s.timestamp <= this.currentTime && s.timestamp > this.currentTime - 60 * 60 * 1000 // 1 hour window
    );

    if (activeScenario) {
      // eslint-disable-next-line functional/immutable-data
      this.mockCVD = {
        aggregatedCVD: activeScenario.globalCVD[activeScenario.globalCVD.length - 1],
        exchangeFlows: [],
        consensus: activeScenario.consensus
          ? activeScenario.globalCVD[activeScenario.globalCVD.length - 1] > 0
            ? 'bullish'
            : 'bearish'
          : 'neutral',
        confidence: 80,
        manipulation: {
          ...
          suspectExchange: activeScenario.manipulationDetected ? 'binance' : null,
          divergenceScore: activeScenario.manipulationDetected ? 90 : 10,
          pattern: activeScenario.manipulationDetected ? 'single_exchange_outlier' : 'none',
        },
  ...
  public validateSignal(
    ...
    if (this.mockCVD) {
      // Simple mock logic
      const isAligned =
        (direction === 'LONG' && this.mockCVD.consensus === 'bullish') ||
        (direction === 'SHORT' && this.mockCVD.consensus === 'bearish');

      const consensusDir =
        this.mockCVD.consensus === 'conflicted' ? 'neutral' : this.mockCVD.consensus;

      return {
        isValid: isAligned,
        adjustedConfidence: isAligned ? technicalConfidence + 10 : technicalConfidence - 20,
        consensusResult: {
          isValid: isAligned,
          hasConsensus: true,
