...
export class ConvictionSizingEngine extends EventEmitter {
  ...
  calculatePositionSize(
    ...
    if (combinedMultiplier > this.config.maxMultiplier) {
      reasoning.push(`Multiplier capped from ${combinedMultiplier.toFixed(2)}x to ${this.config.maxMultiplier}x`);
    }

    // Apply minimum floor
    const finalMultiplier = Math.max(cappedMultiplier, this.config.minMultiplier);
    if (cappedMultiplier < this.config.minMultiplier) {
      reasoning.push(`Multiplier floored from ${cappedMultiplier.toFixed(2)}x to ${this.config.minMultiplier}x`);
    }
  ...
  private calculateOracleMultiplier(oracleScore: OracleScore | null): MultiplierResult {
    ...
    return {
      ...
      reasoning: `Oracle conviction: ${multiplier.toFixed(2)}x (sentiment: ${oracleScore.sentiment})`
    };
  }
  ...
  private calculateFlowMultiplier(flowValidation: FlowValidation | null): MultiplierResult {
    ...
    if (flowValidation.flowType === 'passive_absorption' && flowValidation.institutionalProbability >= 70) {
      multiplier = 1.3;
      reasoning = `Institutional absorption detected (${flowValidation.institutionalProbability.toFixed(0)}% probability)`;
