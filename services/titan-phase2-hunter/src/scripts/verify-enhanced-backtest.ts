...
async function runVerification() {
    ...
    const hologramEngine = new HologramEngine(mockBybit as any);
    ...
    try {
        ...
        console.log(
            `Profit Factor: ${results.metrics.profitFactor.toFixed(2)}`,
        );
        console.log(
            `Final Equity: ${
                results.equityCurve[results.equityCurve.length - 1].equity
                    .toFixed(2)
            }`,
        );
