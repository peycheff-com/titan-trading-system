...
export class HologramScanner extends EventEmitter {
  ...
  public async scan(): Promise<ScanResult> {
    ...
    try {
      ...
      if (scanDuration > this.SCAN_WARNING_THRESHOLD) {
        ...
        console.warn(`⚠️ Slow scan detected: ${scanDuration}ms (threshold: ${this.SCAN_WARNING_THRESHOLD}ms)`);
      }
      ...
      console.log(`✅ Hologram scan complete: ${successCount}/${symbols.length} symbols analyzed in ${scanDuration}ms`);
      console.log(`🎯 Top 20 symbols selected: ${top20.map(h => `${h.symbol}(${h.status})`).join(', ')}`);
  ...
  public selectTop20(rankedSymbols: HologramState[]): HologramState[] {
    ...
    console.warn(`⚠️ Only ${tradeableSymbols.length} tradeable symbols found, selecting top 20 overall`);
  ...
  private async analyzeSymbolsParallel(symbols: string[]): Promise<HologramState[]> {
    ...
    for (let i = 0; i < symbols.length; i += this.MAX_PARALLEL_REQUESTS) {
      ...
      console.log(`📈 Analyzing batch ${Math.floor(i / this.MAX_PARALLEL_REQUESTS) + 1}/${Math.ceil(symbols.length / this.MAX_PARALLEL_REQUESTS)} (${batch.length} symbols)`);
  ...
  public async getSymbolSummary(symbol: string): Promise<string> {
    try {
      ...
      return `❌ ${symbol}: Analysis failed - ${error instanceof Error ? error.message : 'Unknown error'}`;
    }
  }
