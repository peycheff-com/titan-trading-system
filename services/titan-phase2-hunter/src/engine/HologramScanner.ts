...
export class HologramScanner extends EventEmitter {
  ...
  public async scan(): Promise<ScanResult> {
    ...
    try {
      ...
      getLogger().info(
        `🎯 Top 20 symbols selected: ${top20.map(h => `${h.symbol}(${h.status})`).join(', ')}`
      );
  ...
  public rankByAlignment(holograms: HologramState[]): HologramState[] {
    ...
    return holograms.sort((a, b) => {
      ...
      const statusDiff =
        (statusPriority[a.status || 'VETO'] || 99) - (statusPriority[b.status || 'VETO'] || 99);
      if (statusDiff !== 0) {
        return statusDiff;
      }
  ...
  public selectTop20(rankedSymbols: HologramState[]): HologramState[] {
    ...
    const tradeableSymbols = rankedSymbols.filter(
      h => h.status === 'A+' || h.status === 'A' || h.status === 'B'
    );
