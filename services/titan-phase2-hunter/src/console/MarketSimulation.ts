...
export class MarketSimulation {
    ...
    updatePrices(): void {
        this.priceHistory.forEach((history, symbol) => {
            ...
            this.trendMomentum.set(
                symbol,
                Math.max(-0.05, Math.min(0.05, newMomentum)),
            );
        });
    }
    ...
    generateRealisticHolographicMap(): HologramMapEntry[] {
        ...
        return symbols.map((symbol) => {
            ...
            const dailyState = this.generateTimeframeState(
                "1D",
                currentPrice,
                0.15,
            );
            const h4State = this.generateTimeframeState(
                "4H",
                currentPrice,
                0.08,
            );
            const m15State = this.generateTimeframeState(
                "15m",
                currentPrice,
                0.03,
            );
    ...
    private determineHologramStatus(
    ...
    generateRealisticActiveTrade(): ActiveTrade | null {
        ...
        const sessionEvent =
            sessionEvents[Math.floor(Math.random() * sessionEvents.length)];

        // Generate weak levels for targets
        const weakHigh = side === "SHORT"
            ? currentPrice * (1 + Math.random() * 0.02)
            : undefined;
        const weakLow = side === "LONG"
            ? currentPrice * (1 - Math.random() * 0.02)
            : undefined;
        ...
        return {
            ...
            pnl: (currentPrice - entryPrice) * (side === "LONG" ? 1 : -1) *
                0.1 * 4, // 4x leverage
            rValue: ((currentPrice - entryPrice) / (entryPrice * 0.015)) *
                (side === "LONG" ? 1 : -1),
    ...
    generateRealisticPOIMap(): POIMapEntry[] {
        ...
        for (let i = 0; i < 3; i++) {
            const direction: "BULLISH" | "BEARISH" = Math.random() > 0.5
                ? "BULLISH"
                : "BEARISH";
    ...
    generateRealisticSessionState(): EnhancedSessionState {
        ...
        const startTime = Date.now() -
            Math.random() * durations[currentSession];
        ...
        const judasSwing =
            (currentSession === "LONDON" || currentSession === "NY") &&
                Math.random() > 0.6
                ? {
                    type: Math.random() > 0.5
                        ? "SWEEP_HIGH"
                        : ("SWEEP_LOW" as "SWEEP_HIGH" | "SWEEP_LOW"),
        ...
        return {
            ...
            asianRange,
            judasSwing,
            killzoneActive: currentSession === "LONDON" ||
                currentSession === "NY",
            volumeProfile: currentSession === "DEAD_ZONE"
                ? "LOW"
