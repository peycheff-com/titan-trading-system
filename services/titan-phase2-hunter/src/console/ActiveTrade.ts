...
export class ActiveTradeComponent {
  ...
  render(): string[] {
    ...
    if (this.trade) {
      ...
      lines.push(`\x1b[1m${this.trade.symbol} ${this.trade.side}\x1b[0m ${rColor}${this.formatRValue(this.trade.rValue)}\x1b[0m`);
  ...
  private formatNarrative(narrative: ActiveTradeData['narrative']): string {
    const biasColor = narrative.dailyBias === 'BULL' ? '\x1b[32m' : 
                     narrative.dailyBias === 'BEAR' ? '\x1b[31m' : '\x1b[33m';
    
    const locationColor = narrative.h4Location === 'DISCOUNT' ? '\x1b[32m' : 
                         narrative.h4Location === 'PREMIUM' ? '\x1b[31m' : '\x1b[33m';
  ...
  private formatSetup(setup: ActiveTradeData['setup']): string {
    const typeDisplay = setup.type === 'OB' ? 'Order Block' :
                       setup.type === 'FVG' ? 'Fair Value Gap' : 'Liquidity Pool';
    
    const confidenceColor = setup.confidence >= 80 ? '\x1b[32m' : 
                           setup.confidence >= 60 ? '\x1b[33m' : '\x1b[31m';
  ...
  private formatConfirmation(confirmation: ActiveTradeData['confirmation']): string {
    const eventDisplay = confirmation.sessionEvent === 'JUDAS_SWING' ? 'Judas Swing' :
                        confirmation.sessionEvent === 'KILLZONE_ENTRY' ? 'Killzone Entry' : 'Session Open';
    
    const cvdColor = confirmation.cvdStatus === 'ABSORPTION' ? '\x1b[32m' : 
                    confirmation.cvdStatus === 'DISTRIBUTION' ? '\x1b[31m' : '\x1b[33m';
  ...
  private formatExecution(execution: ActiveTradeData['execution']): string {
    const slippageColor = Math.abs(execution.slippage) <= 0.1 ? '\x1b[32m' : 
                         Math.abs(execution.slippage) <= 0.2 ? '\x1b[33m' : '\x1b[31m';
  ...
  private formatTarget(targets: ActiveTradeData['targets'], side: 'LONG' | 'SHORT'): string {
    ...
    const weakLevelText = weakLevel ? `Weak ${side === 'LONG' ? 'Low' : 'High'} ${this.formatPrice(weakLevel)}` : 'No weak level';
    
    const stopText = `SL ${this.formatPrice(targets.stopLoss)}`;
    const targetText = `TP ${this.formatPrice(targets.takeProfit)}`;
    
    const statusText = targets.breakeven ? '\x1b[32m[BE]\x1b[0m' : 
                      targets.trailingActive ? '\x1b[33m[TRAIL]\x1b[0m' : '';
