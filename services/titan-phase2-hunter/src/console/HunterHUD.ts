...
export class HunterHUD {
  ...
  private updateEquityAndPnL(): void {
    ...
    if (this.hudState.activeTrade) {
      ...
      const pnlChange =
        priceDiff * trade.quantity * trade.leverage * (trade.side === 'LONG' ? 1 : -1);

      trade.currentPrice = newPrice;
      trade.pnl += pnlChange;
      trade.rValue =
        ((newPrice - trade.entryPrice) / (trade.entryPrice * 0.015)) *
        (trade.side === 'LONG' ? 1 : -1);
    ...
    // Update percentage
    this.hudState.pnlPercent =
      (this.hudState.pnl / (this.hudState.equity - this.hudState.pnl)) * 100;

    // Update portfolio heat and max drawdown
    this.hudState.portfolioHeat = Math.max(
      0,
      Math.min(20, this.hudState.portfolioHeat + (Math.random() - 0.5) * 0.5)
    );
  ...
  private updateSessionState(): void {
    this.hudState.sessionState.timeRemaining = Math.max(
      0,
      this.hudState.sessionState.timeRemaining - 1000
    );
    ...
    this.hudState.sessionState.volumeProfile =
      session === 'DEAD_ZONE' ? 'LOW' : session === 'ASIAN' ? 'MEDIUM' : 'HIGH';
  ...
  private updateActiveTrade(): void {
    ...
    if (
      (trade.side === 'LONG' && trade.currentPrice <= trade.targets.stopLoss) ||
    ...
    if (
      (trade.side === 'LONG' && trade.currentPrice >= trade.targets.takeProfit) ||
  ...
  private updateHolographicMap(): void {
    this.hudState.holographicMap.forEach(entry => {
      ...
      entry.alignmentScore = this.calculateAlignmentScore(
        entry.dailyState,
        entry.h4State,
        entry.m15State
      );
      ...
      if (Math.random() < 0.02 && entry.status === 'A+') {
        // 2% chance for A+ setups
        entry.lastSignal = {
  ...
  private updatePOIMap(): void {
    ...
    this.hudState.poiMap = this.hudState.poiMap.filter(
      poi => poi.age < 72 && (!poi.mitigated || poi.age < 24)
    );
  ...
  private updateSystemHealth(): void {
    ...
    if (Math.random() < 0.001) {
      ...
      setTimeout(() => {
        health.wsConnections.binance = true;
      }, 5000);
    }
    ...
    if (Math.random() < 0.001) {
      ...
      setTimeout(() => {
        health.wsConnections.bybit = true;
      }, 3000);
    }
  ...
  private getStatusColor(status: HologramStatus): string {
    switch (status) {
      case 'A+':
        return '\x1b[1m\x1b[32m'; // Bold Bright Green
      case 'B':
  ...
  private getSessionColor(sessionType: SessionType): string {
    switch (sessionType) {
      case 'LONDON':
        return '\x1b[1m\x1b[32m'; // Bold Green (active trading)
      case 'NY':
        return '\x1b[1m\x1b[34m'; // Bold Blue (active trading)
      case 'ASIAN':
        return '\x1b[33m'; // Yellow (moderate activity)
      case 'DEAD_ZONE':
        return '\x1b[90m'; // Dark Gray (low activity)
      default:
        return '\x1b[0m'; // Reset
    }
  }
  ...
  private getLocationColor(location: 'PREMIUM' | 'DISCOUNT' | 'EQUILIBRIUM'): string {
    switch (location) {
      case 'PREMIUM':
        return '\x1b[31m'; // Red (sell zone)
      case 'DISCOUNT':
        return '\x1b[32m'; // Green (buy zone)
      case 'EQUILIBRIUM':
  ...
  private render(): void {
    ...
    console.log(
      `│ 🎯 ${this.hudState.phase} | Equity: ${equityColor}${this.formatCurrency(this.hudState.equity)}\x1b[0m | P&L: ${equityColor}${this.formatCurrency(this.hudState.pnl)} (${this.formatPercent(this.hudState.pnlPercent)})\x1b[0m │`
    );
    console.log(
      `│ Session: ${sessionColor}${this.hudState.sessionState.type}\x1b[0m${killzoneIndicator} | Time: ${this.formatTime(this.hudState.sessionState.timeRemaining)} | Vol: ${this.hudState.sessionState.volumeProfile} | Heat: ${this.formatPercent(this.hudState.portfolioHeat)}${pauseIndicator} │`
    );
    console.log(
      '└─────────────────────────────────────────────────────────────────────────────────┘'
    );
    ...
    console.log(
      `[F1] CONFIG  [F2] VIEW (${this.hudState.viewMode})  [SPACE] ${this.hudState.isPaused ? 'RESUME' : 'PAUSE'}  [Q] QUIT | WS: ${binanceStatus}BIN\x1b[0m ${bybitStatus}BYB\x1b[0m | Scan: ${this.hudState.systemHealth.scanDuration.toFixed(1)}s`
    );
    console.log('');
    ...
    console.log(
      '│        📊 HOLOGRAPHIC MAP        │         🎯 ACTIVE TRADE          │          📍 POI MAP             │'
    );
    console.log(
      '├─────────────────────────────────┼─────────────────────────────────┼─────────────────────────────────┤'
    );
    ...
    console.log(
      '│ Symbol    Trend  Loc  Trig  RS  │                                 │ Type      Price     Dist   Conf │'
    );
    console.log(
      '├─────────────────────────────────┼─────────────────────────────────┼─────────────────────────────────┤'
    );
    ...
    for (
      let i = 0;
      i < Math.max(displaySymbols.length, activeTradeLines.length - 1, poiMapLines.length - 1, 8);
      i++
    ) {
    ...
    const trendColor =
      this.hudState.marketConditions.trend === 'BULL_MARKET'
        ? '\x1b[32m'
        : this.hudState.marketConditions.trend === 'BEAR_MARKET'
          ? '\x1b[31m'
          : '\x1b[33m';
    const volColor =
      this.hudState.marketConditions.volatility === 'HIGH'
        ? '\x1b[31m'
        : this.hudState.marketConditions.volatility === 'MEDIUM'
          ? '\x1b[33m'
          : '\x1b[32m';

    console.log(
      `Market: ${trendColor}${this.hudState.marketConditions.trend}\x1b[0m | Vol: ${volColor}${this.hudState.marketConditions.volatility}\x1b[0m | BTC Dom: ${this.hudState.marketConditions.btcDominance.toFixed(1)}% | F&G: ${this.hudState.marketConditions.fearGreedIndex.toFixed(0)}`
    );
    console.log(
      `Positions: ${this.hudState.positions.length} | Max DD: ${this.formatPercent(this.hudState.maxDrawdown)} | Errors: ${this.hudState.systemHealth.errorCount} | Updated: ${new Date().toLocaleTimeString()}`
    );
  }
}
