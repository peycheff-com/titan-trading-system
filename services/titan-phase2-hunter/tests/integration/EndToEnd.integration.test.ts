  ...
  private initializeMockData(): void {
    ...
    for (const symbol of mockTopSymbols) {
      if (!this.mockData.has(`${symbol}-1D`)) {
        const multiplier = symbol === "BTCUSDT"
          ? 1
          : symbol === "ETHUSDT"
          ? 0.07
          : 0.001;
...
describe("End-to-End Integration Tests", () => {
  ...
  describe("Complete End-to-End Flow", () => {
    it(
      "should execute full cycle: Hologram Scan → Session Check → POI Detection → CVD Validation → Signal Generation → Execution",
      async () => {
        ...
        console.log(
          `✅ Hologram scan complete: ${scanResult.successCount} symbols analyzed`,
        );
        ...
        expect(["ASIAN", "LONDON", "NY", "DEAD_ZONE"]).toContain(
          sessionState.type,
        );
        ...
        const candles = await mockBybitClient.fetchOHLCV(
          bestHologram.symbol,
          "15m",
          100,
        );
        ...
        const cvdValues = Array(prices.length).fill(0).map((_, i) =>
          cvd + (Math.random() - 0.5) * 100
        );
        const absorption = cvdValidator.detectAbsorption(prices, cvdValues);

        console.log(
          `✅ CVD validation complete: CVD=${cvd.toFixed(2)}, Absorption=${
            absorption ? "detected" : "none"
          }`,
        );
        ...
        if (bestHologram.alignment === "A+" || bestHologram.alignment === "B") {
          ...
          // Generate signal using the actual SignalGenerator logic
          // This validates Oracle and Global CVD as well
          ...
          // We expect a signal if conditions are met
          if (signal) {
            const mockSignal = signal; // Use the generated signal
    ...
    it("should handle hologram scan with multiple symbols", async () => {
      ...
      // Verify scan results
      expect(scanResult.symbols.length).toBeGreaterThanOrEqual(
        mockTopSymbols.length,
      );
      expect(scanResult.successCount).toBeGreaterThan(0);
      ...
      console.log(
        `✅ Multi-symbol scan complete: ${scanResult.symbols.length} symbols processed`,
      );
    });
    ...
    it("should validate session transitions and killzone detection", async () => {
      ...
      console.log(
        `✅ Session validation complete: ${currentSession.type} session, killzone=${isKillzone}`,
      );
    });
    ...
    it("should detect and validate POIs correctly", async () => {
      ...
      console.log(
        `✅ POI detection complete: ${fvgs.length} FVGs, ${orderBlocks.length} Order Blocks`,
      );
    });
    ...
    it("should calculate CVD and detect absorption patterns", async () => {
      ...
      const distribution = cvdValidator.detectDistribution(
        distributionPrices,
        distributionCVD,
      );
  ...
  describe("Performance and Reliability", () => {
    it("should complete hologram scan within reasonable time", async () => {
      ...
      console.log(
        `⏱️ Scan performance: ${duration}ms (${scanResult.symbols.length} symbols)`,
      );
    });
    ...
    it("should maintain data consistency across components", async () => {
      ...
      expect(hologram.daily.dealingRange.high).toBeGreaterThan(
        hologram.daily.dealingRange.low,
      );
      expect(hologram.h4.dealingRange.high).toBeGreaterThan(
        hologram.h4.dealingRange.low,
      );
      expect(hologram.m15.dealingRange.high).toBeGreaterThan(
        hologram.m15.dealingRange.low,
      );
