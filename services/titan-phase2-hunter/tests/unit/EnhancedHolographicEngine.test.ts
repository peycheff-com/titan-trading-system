...
function createMockGlobalCVD(
  ...
  return {
    ...
    exchangeFlows: [
      { exchange: 'binance', cvd: 500, volume: 1000000, trades: 5000, weight: 0.4, timestamp: new Date(), status: 'connected' as any },
      { exchange: 'coinbase', cvd: 300, volume: 800000, trades: 4000, weight: 0.35, timestamp: new Date(), status: 'connected' as any },
      { exchange: 'kraken', cvd: 200, volume: 500000, trades: 2000, weight: 0.25, timestamp: new Date(), status: 'connected' as any }
    ],
    consensus,
    confidence,
    manipulation: {
  ...
  describe('calculateEnhancedScore', () => {
    ...
    test('should apply bot trap penalty', () => {
      ...
      const cleanResult = scoringEngine.calculateEnhancedScore(
        hologram, oracle, flow, botTrapClean, globalCVD
      );

      const suspectResult = scoringEngine.calculateEnhancedScore(
        hologram, oracle, flow, botTrapSuspect, globalCVD
      );

      expect(suspectResult.adjustedScore).toBeLessThan(cleanResult.adjustedScore);
...
describe('ConvictionSizingEngine', () => {
  ...
  describe('calculatePositionSize', () => {
    ...
    test('should use conservative multiplier on conflicts (Requirement 7.6)', () => {
      ...
      expect(result.reasoning).toContain('Using conservative multiplier selection');
    });
...
describe('EnhancedSignalValidator', () => {
  ...
  describe('validateSignal', () => {
    ...
    test('should adjust confidence based on layer validations', () => {
      ...
      expect(badResult.adjustedConfidence).toBeLessThan(goodResult.adjustedConfidence);
    });
  });
  ...
  describe('conflict resolution', () => {
    ...
    test('should detect conflicts between layers', () => {
      ...
      expect(result.conflictAnalysis.hasConflict).toBe(true);
      expect(result.conflictAnalysis.conflictingLayers.length).toBeGreaterThan(0);
    });
  });
});
