...
import { OHLCV, TimeframeState, HologramState, VetoResult } from '../../src/types';
...
describe('HologramEngine', () => {
  ...
  describe('calcAlignmentScore', () => {
    it('should calculate maximum score for full alignment', () => {
      const daily: TimeframeState = {
        timeframe: '1D',
        trend: 'BULL',
      ...
      const h4: TimeframeState = {
        timeframe: '4H',
        trend: 'BULL', // Same as daily (50 points)
      ...
      const score = hologramEngine.calcAlignmentScore(daily, h4, m15);
      expect(score).toBe(100); // 50 + 30 + 20 = 100
    });

    it('should calculate partial score for partial alignment', () => {
      const daily: TimeframeState = {
        timeframe: '1D',
        trend: 'BULL',
        dealingRange: { high: 120, low: 80, midpoint: 100, premiumThreshold: 100, discountThreshold: 100, range: 40 },
      ...
      const h4: TimeframeState = {
        timeframe: '4H',
        trend: 'BEAR', // Different from daily (0 points)
        dealingRange: { high: 115, low: 85, midpoint: 100, premiumThreshold: 100, discountThreshold: 100, range: 30 },
        currentPrice: 110,
        location: 'PREMIUM',
        fractals: [],
      ...
      const m15: TimeframeState = {
        timeframe: '15m',
        trend: 'BEAR', // Same as 4H (30 points)
        dealingRange: { high: 112, low: 88, midpoint: 100, premiumThreshold: 100, discountThreshold: 100, range: 24 },
        currentPrice: 110,
        location: 'PREMIUM',
        fractals: [],
        bos: [],
        mss: null // No MSS (0 points)
      };

      const score = hologramEngine.calcAlignmentScore(daily, h4, m15);
      expect(score).toBe(30); // 0 + 30 + 0 = 30
    });

    it('should not award points for RANGE trends', () => {
      const daily: TimeframeState = {
        timeframe: '1D',
        trend: 'RANGE',
        dealingRange: { high: 120, low: 80, midpoint: 100, premiumThreshold: 100, discountThreshold: 100, range: 40 },
      ...
      const h4: TimeframeState = {
        timeframe: '4H',
        trend: 'RANGE', // Both RANGE (0 points)
        dealingRange: { high: 115, low: 85, midpoint: 100, premiumThreshold: 100, discountThreshold: 100, range: 30 },
        currentPrice: 110,
        location: 'PREMIUM',
        fractals: [],
      ...
      const score = hologramEngine.calcAlignmentScore(daily, h4, m15);
      expect(score).toBe(20); // 0 + 0 + 20 = 20
    });
  });

  describe('applyVetoLogic', () => {
    it('should veto long when daily bullish but 4H in premium', () => {
      const daily: TimeframeState = {
        timeframe: '1D',
        trend: 'BULL',
        dealingRange: { high: 120, low: 80, midpoint: 100, premiumThreshold: 100, discountThreshold: 100, range: 40 },
      ...
      const h4: TimeframeState = {
        ...
        dealingRange: { high: 115, low: 85, midpoint: 100, premiumThreshold: 100, discountThreshold: 100, range: 30 },
        currentPrice: 110,
        location: 'PREMIUM', // In premium zone
        fractals: [],
        bos: [],
        mss: null
      };

      const veto = hologramEngine.applyVetoLogic(daily, h4);
      
      expect(veto.vetoed).toBe(true);
      expect(veto.direction).toBe('LONG');
      expect(veto.reason).toContain('too expensive to buy');
    });

    it('should veto short when daily bearish but 4H in discount', () => {
      const daily: TimeframeState = {
        timeframe: '1D',
        trend: 'BEAR',
        dealingRange: { high: 120, low: 80, midpoint: 100, premiumThreshold: 100, discountThreshold: 100, range: 40 },
      ...
      const h4: TimeframeState = {
        ...
        dealingRange: { high: 115, low: 85, midpoint: 100, premiumThreshold: 100, discountThreshold: 100, range: 30 },
        currentPrice: 90,
        location: 'DISCOUNT', // In discount zone
        fractals: [],
        bos: [],
        mss: null
      };
      ...
      expect(veto.vetoed).toBe(true);
      expect(veto.direction).toBe('SHORT');
      expect(veto.reason).toContain('too cheap to sell');
    });

    it('should not veto when daily bullish and 4H in discount', () => {
      const daily: TimeframeState = {
        timeframe: '1D',
        trend: 'BULL',
        dealingRange: { high: 120, low: 80, midpoint: 100, premiumThreshold: 100, discountThreshold: 100, range: 40 },
      ...
      const h4: TimeframeState = {
        ...
        dealingRange: { high: 115, low: 85, midpoint: 100, premiumThreshold: 100, discountThreshold: 100, range: 30 },
        currentPrice: 90,
        location: 'DISCOUNT', // Good for buying
        fractals: [],
        bos: [],
        mss: null
      };
      ...
      expect(veto.direction).toBe(null);
      expect(veto.reason).toBe(null);
    });

    it('should not veto when daily bearish and 4H in premium', () => {
      const daily: TimeframeState = {
        timeframe: '1D',
        trend: 'BEAR',
        dealingRange: { high: 120, low: 80, midpoint: 100, premiumThreshold: 100, discountThreshold: 100, range: 40 },
      ...
      const h4: TimeframeState = {
        ...
        dealingRange: { high: 115, low: 85, midpoint: 100, premiumThreshold: 100, discountThreshold: 100, range: 30 },
        currentPrice: 110,
        location: 'PREMIUM', // Good for selling
        fractals: [],
        bos: [],
        mss: null
      };
  ...
  describe('getHologramStatus', () => {
    it('should return NO_PLAY when vetoed regardless of score', () => {
      ...
      const status = hologramEngine.getHologramStatus(100, veto);
      expect(status).toBe('NO_PLAY');
    });

    it('should return A+ for score >= 80 without veto', () => {
      const veto: VetoResult = {
        vetoed: false,
        reason: null,
        direction: null
      ...
      expect(hologramEngine.getHologramStatus(80, veto)).toBe('A+');
      expect(hologramEngine.getHologramStatus(90, veto)).toBe('A+');
      expect(hologramEngine.getHologramStatus(100, veto)).toBe('A+');
    });

    it('should return B for score 60-79 without veto', () => {
      const veto: VetoResult = {
        vetoed: false,
        reason: null,
        direction: null
      ...
      expect(hologramEngine.getHologramStatus(60, veto)).toBe('B');
      expect(hologramEngine.getHologramStatus(70, veto)).toBe('B');
      expect(hologramEngine.getHologramStatus(79, veto)).toBe('B');
    });

    it('should return CONFLICT for score < 60 without veto', () => {
      const veto: VetoResult = {
        vetoed: false,
        reason: null,
        direction: null
      ...
      expect(hologramEngine.getHologramStatus(0, veto)).toBe('CONFLICT');
      expect(hologramEngine.getHologramStatus(30, veto)).toBe('CONFLICT');
      expect(hologramEngine.getHologramStatus(59, veto)).toBe('CONFLICT');
    });
  });
  ...
  describe('analyze', () => {
    ...
    it('should throw error on API failure', async () => {
      ...
      await expect(hologramEngine.analyze('ETHUSDT')).rejects.toThrow('Failed to analyze hologram for ETHUSDT');
    });
  });
  ...
  describe('validateHologramState', () => {
    it('should validate correct hologram state', () => {
      ...
      expect(() => HologramEngine.validateHologramState(validHologram)).not.toThrow();
    });
    ...
    it('should throw error for invalid alignment score', () => {
      const invalidHologram = {
        ...
        status: 'A+' as const,
        daily: { fractals: [], bos: [], trend: 'BULL' as const, location: 'PREMIUM' as const, timeframe: '1D' as const },
        h4: { fractals: [], bos: [], trend: 'BULL' as const, location: 'PREMIUM' as const, timeframe: '4H' as const },
        m15: { fractals: [], bos: [], trend: 'BULL' as const, location: 'PREMIUM' as const, timeframe: '15m' as const },
        veto: { vetoed: false, reason: null, direction: null },
        rsScore: 0.05
      } as any;

      expect(() => HologramEngine.validateHologramState(invalidHologram)).toThrow('alignment score must be 0-100');
    });
    ...
    it('should throw error for invalid status', () => {
      ...
      expect(() => HologramEngine.validateHologramState(invalidHologram)).toThrow('invalid status');
    });
  });
