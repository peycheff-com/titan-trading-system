...
describe("HologramScanner", () => {
  ...
  beforeEach(() => {
    ...
    mockHologramEngine = (scanner as any).hologramEngine as jest.Mocked<
      HologramEngine
    >;

    // Mock the static method
    jest.spyOn(HologramEngine, "getHologramSummary").mockImplementation(
      (hologram) => {
        return `🟢 ${hologram.symbol} | Score: ${
          hologram.enhancedScore || hologram.alignmentScore
        } | RS: ${(hologram.rsScore * 100).toFixed(1)}%`;
      },
    );
  ...
  describe("rankByAlignment", () => {
    it("should rank symbols by status priority then alignment score", () => {
      const holograms: HologramState[] = [
        ...
        createMockHologram("SYMBOL5", "C", 95),
        createMockHologram("SYMBOL6", "B", 65),
      ];

      const ranked = scanner.rankByAlignment(holograms);

      // Expected order: A+ (90), A+ (85), B (70), B (65), C (95), VETO (30)
      ...
      expect(ranked[4].symbol).toBe("SYMBOL5"); // C 95
      expect(ranked[5].symbol).toBe("SYMBOL1"); // VETO 30
    });
  ...
  describe("selectTop20", () => {
    it("should select top 20 tradeable symbols (A+ and B)", () => {
      ...
      // Create 5 VETO symbols
      for (let i = 1; i <= 5; i++) {
        holograms.push(createMockHologram(`C_SYMBOL${i}`, "VETO", 30 - i));
      }
  ...
  describe("scan", () => {
    ...
    it("should complete successful scan", async () => {
      ...
      mockHologramEngine.analyze
        ...
        .mockResolvedValueOnce(createMockHologram("DOTUSDT", "VETO", 30))
        .mockResolvedValueOnce(createMockHologram("LINKUSDT", "C", 95));
    ...
    it("should emit scanError event on failure", async () => {
      mockBybitClient.fetchTopSymbols.mockRejectedValue(
        new Error("API Error"),
      );
  ...
  describe("getSymbolSummary", () => {
    ...
    it("should return error message for failed analysis", async () => {
      mockHologramEngine.analyze.mockRejectedValue(
  ...
  describe("error handling", () => {
    ...
    it("should handle empty symbol list", async () => {
      ...
      await expect(scanner.scan()).rejects.toThrow(
        "No symbols returned from exchange",
      );
    });
