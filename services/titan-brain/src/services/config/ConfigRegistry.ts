...
export class ConfigRegistry {
    ...
    async initialize(): Promise<void> {
        ...
        try {
            ...
            this.logger.error(
                "Failed to load config persistence",
                error as Error,
            );
    ...
    getEffective(key: string): EffectiveConfig | null {
        ...
        const effectiveValue = provenance.length > 0
            ? provenance[provenance.length - 1].value
            : null;
    ...
    private checkSafety(
        ...
        if (item.safety === "immutable") {
            return {
                allowed: false,
                reason:
                    "This setting is immutable and requires a signed deploy",
            };
        }
            ...
            if (item.riskDirection === "higher_is_riskier") {
                ...
                if (proposed > current) {
                    return {
                        allowed: false,
                        reason:
                            `Tighten-only: Cannot increase ${item.key} from ${current} to ${proposed}`,
                    };
                }
                ...
                if (proposed < current) {
                    return {
                        allowed: false,
                        reason:
                            `Tighten-only: Cannot decrease ${item.key} from ${current} to ${proposed}`,
                    };
                }
            }
            return { allowed: true };
        ...
        if (item.safety === "raise_only") {
            ...
            if (proposed < current) {
                return {
                    allowed: false,
                    reason:
                        `Raise-only: Cannot lower ${item.key} from ${current} to ${proposed}`,
                };
            }
            return { allowed: true };
        }
        ...
        if (item.safety === "append_only") {
            ...
            const removedItems = currentArr.filter((item) =>
            ...
            if (removedItems.length > 0) {
                return {
                    allowed: false,
                    reason:
                        `Append-only: Cannot remove items from ${item.key}: ${
                            removedItems.join(", ")
                        }`,
                };
            }
     ...
     */
    private async deactivateOverrideInDb(
        key: string,
        operatorId: string,
    ): Promise<void> {
        if (!this.pool) return;
