...
export class RegimeInferenceService {
  ...
  public async start(): Promise<void> {
    ...
    this.nc.subscribe(TITAN_SUBJECTS.DATA.POWERLAW.ALL, async (data: unknown) => {
      try {
  ...
  private processMetrics(metrics: PowerLawMetricsV1): void {
    ...
    if (metrics.health.status !== 'ok' && metrics.health.status !== 'stale') {
  ...
  private publishRegimeChange(): void {
    this.nc.publish(TITAN_SUBJECTS.EVT.BRAIN.REGIME, {
