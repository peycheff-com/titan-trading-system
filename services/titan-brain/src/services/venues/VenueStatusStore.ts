...
export class VenueStatusStore extends EventEmitter {
    ...
    async start(): Promise<void> {
        ...
        this.logger.info(
            `Subscribing to venue status updates on ${this.config.subject}`,
        );
                ...
                for await (const entry of watcher) {
                    ...
                    if (operation === "DEL" || operation === "PURGE") {
                        ...
                        if (result.success) {
                            ...
                            this.logger.warn(
                                `Invalid venue config for ${venueId}: ${
    ...
    async bootstrapFromKV(): Promise<void> {
        ...
        try {
            ...
            this.logger.info(
                `KV bootstrap: found ${keys.length} venue snapshots`,
            );
            ...
            for (const key of keys) {
                ...
                if (data) {
                    ...
                    if (result.success) {
                        ...
                        const now = new Date();
                        const threshold = this.getThresholdForVenue(
                            result.data.venue,
                        );
                        ...
                        this.logger.debug(
                            `KV bootstrap: loaded ${result.data.venue}`,
                        );
                    }
                }
            }
    ...
    private handleMessage(data: unknown): void {
        ...
        if (this.metricsCollector) {
            const stateStr = wsState === VenueWsState.CONNECTED
                ? "connected"
                ...
                ? "disconnected"
                : "unknown";
            this.metricsCollector.recordVenueStatus(
                status.venue,
                stateStr,
                false,
            );
    ...
    getSummary(): {
        ...
        if (this.metricsCollector) {
            this.metricsCollector.recordVenueSummary(
                connected,
                ALL_VENUE_IDS.length,
            );
        }
    ...
    private evictStaleEntries(): void {
        ...
        const evictBefore = new Date(
            now.getTime() - this.config.evictAfterStalenessMs,
        );
