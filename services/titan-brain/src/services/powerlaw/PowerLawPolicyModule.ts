...
export class PowerLawPolicyModule {
  ...
  async processMetrics(metrics: PowerLawMetricsV1): Promise<void> {
    ...
    // Determine impact
    const impact = this.assessImpact(previousConstraints, constraints, metrics);
    ...
    switch (this.config.mode) {
      ...
      case 'ADVISORY':
        ...
        if (impact.severity !== 'NONE') {
          logger.info(`[ADVISORY] PowerLaw impact for ${key}: ${impact.reason}`);
        }
      ...
      case 'ENFORCEMENT':
        ...
        if (impact.severity === 'CRITICAL') {
          logger.warn(`[ENFORCEMENT] Critical PowerLaw impact for ${key}: ${impact.reason}`);
        }
        break;
    }
  }

  private async publishConstraints(constraints: ExecutionConstraintsV1): Promise<void> {
  ...
  private assessImpact(
    ...
    if (alpha !== null && alpha < this.config.alphaCritical) {
      ...
      return {
        ...
        severity: 'CRITICAL',
        reason: `Tail alpha ${alpha.toFixed(
          3,
    ...
    if (exceedance >= this.config.exceedanceCritical) {
      ...
      return {
        type: 'CONSTRAINT_TIGHTENED',
        severity: 'CRITICAL',
        reason: `Exceedance probability ${(exceedance * 100).toFixed(1)}% above critical threshold`,
        reasonCodes,
      };
    }
    ...
    if (previous) {
      if (current.limits.max_order_notional < previous.limits.max_order_notional) {
        reasonCodes.push('NOTIONAL_REDUCED');
        return {
          type: 'CONSTRAINT_TIGHTENED',
          severity: 'INFO',
          reason: `Max notional reduced from ${previous.limits.max_order_notional} to ${current.limits.max_order_notional}`,
          reasonCodes,
        };
      }
      if (current.limits.max_order_notional > previous.limits.max_order_notional) {
        ...
        return {
          ...
          severity: 'INFO',
          reason: `Max notional increased from ${previous.limits.max_order_notional} to ${current.limits.max_order_notional}`,
          reasonCodes,
        };
      }
    }
   ...
   */
  getConstraints(venue: string, symbol: string): ExecutionConstraintsV1 | null {
    const state = this.symbolState.get(`${venue}:${symbol}`);
    if (!state?.lastConstraints) return null;

    // Check TTL
    if (Date.now() - state.constraintsIssuedAt > this.config.constraintsTTLMs) {
      return null; // Expired
    }
  ...
  getAllConstraints(): ExecutionConstraintsV1[] {
    ...
    for (const state of this.symbolState.values()) {
      if (state.lastConstraints && now - state.constraintsIssuedAt < this.config.constraintsTTLMs) {
...
export class ExecutionConstraintsBuilder {
  ...
  build(metrics: PowerLawMetricsV1): ExecutionConstraintsV1 {
    ...
    if (alpha < this.config.alphaCritical || exceedance >= this.config.exceedanceCritical) {
      riskMode = 'EMERGENCY';
    } else if (alpha < this.config.alphaWarning || exceedance >= this.config.exceedanceWarning) {
      riskMode = 'CAUTION';
    } else if (inCluster) {
      riskMode = 'DEFENSIVE';
    }
    ...
    const now = Date.now();
    const reasonCodes = this.buildReasonCodes(alpha, confidence, exceedance, inCluster);
    ...
    const constraints: ExecutionConstraintsV1 = {
      ...
      limits: {
        max_pos_notional: Math.round(this.config.defaultMaxPosNotional * notionalScale),
        max_order_notional: Math.round(this.config.defaultMaxOrderNotional * notionalScale),
        max_leverage: parseFloat((this.config.defaultMaxLeverage * leverageScale).toFixed(2)),
        reduce_only: riskMode === 'EMERGENCY',
      },
      execution_profile: {
        slicing: {
          max_slice_notional: Math.round(this.config.defaultSliceNotional * notionalScale),
