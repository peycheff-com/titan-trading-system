...
async function verifySignalFlow() {
    ...
    const config = configManager
        .getBrainConfig() as unknown as TitanBrainConfig;
    ...
    try {
        ...
        console.error(
            "❌ Failed to connect to IPC. Is titan-execution-rs running?",
        );
        process.exit(1);
    }
    ...
    const fillPromise = new Promise<FillReport>((resolve, reject) => {
        const timeout = setTimeout(
            () => reject(new Error("Timeout waiting for FillReport")),
            10000,
        );

        nats.subscribe(`titan.execution.fill.${symbol}`, (msg: FillReport) => {
    ...
    try {
        ...
        if (!fill.client_order_id || fill.client_order_id.length === 0) {
            throw new Error(
                "❌ client_order_id is MISSING or empty in FillReport",
            );
        }
        ...
        if (count !== 1) {
            throw new Error(
                `❌ Database verification failed. Expected 1 record, found ${count}`,
            );
        }

        // detailed check
        const record = await dbManager.queryOne<any>(
            `SELECT * FROM fills WHERE signal_id = $1`,
            [testSignalId],
        );
