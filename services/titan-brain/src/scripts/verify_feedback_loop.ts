...
function sign(payload: any) {
  ...
  const signature = createHmac("sha256", SECRET).update(data, "utf8").digest(
    "hex",
  );
...
async function main() {
  ...
  try {
    ...
    if (!response1.ok) {
      ...
      throw new Error(
        `Failed to send report: ${response1.statusText} - ${text}`,
      );
    }
    ...
    if (!response2.ok) {
      ...
      throw new Error(
        `Failed to send close report: ${response2.statusText} - ${text}`,
      );
    }
    console.log("SUCCESS: Accepted Sell Report");
    console.log(
      "Check Titan Brain logs for PnL calculation: Should see ~$1000 profit.",
    );
  } catch (error) {
