...
import { RegimeInferenceService } from '../services/RegimeInferenceService.js';
...
export class TitanBrain implements PositionClosureHandler, BreakerEventPersistence {
  ...
  private regimeInferenceService: RegimeInferenceService | null = null;
  ...
  constructor(
    ...
    this.riskManager = new RiskManager(this.config.brain as any, getNatsPublisher());
  ...
  private handleLeadershipPromotion = async () => {
    logger.info('👑 Brain promoted to LEADER. Verifying Execution policy hash...');

    // P0: Policy Hash Handshake - Verify Execution has same policy before processing
    const { hash } = getCanonicalRiskPolicy();
    const handshakeResult = await verifyExecutionPolicyHash(this.natsClient, hash, 5000, 3);

    if (!handshakeResult.success) {
      logger.error(`🚨 CRITICAL: Policy hash handshake FAILED. ${handshakeResult.error}`);
      logger.error(
    ...
    logger.info(`✅ Policy hash handshake OK. Enabling signal processing. Hash: ${hash}`);
  ...
  private handleLeadershipDemotion = () => {
    logger.info('🙇 Brain demoted to FOLLOWER. Disabling signal processing and active services.');
    if (this.signalProcessor) {
  ...
  async start(): Promise<void> {
    try {
      ...
      logger.info(`Titan Brain started successfully. Risk Policy v${version} Hash: ${hash}`);
  ...
  async initialize(): Promise<void> {
    ...
    if (
      ...
      this.reconciliationService.setDriftListener(
        (driftDetected: boolean, severity?: string, reason?: string) => {
          this.riskGuardian.updateConfidence(driftDetected);
          if (driftDetected && severity === 'CRITICAL') {
            this.riskGuardian.tripCircuitBreaker(reason || 'Critical Drift Detected');
          }
        },
      );
    ...
    this.natsClient.subscribe(TITAN_SUBJECTS.CMD.OPERATOR.ALL, async (msg: any) => {
      await this.handleOperatorAction(msg.payload || msg);
    });

    // 4. Subscribe to Truth Snapshots (P0: Truth Dominance)
    this.natsClient.subscribe(TITAN_SUBJECTS.EVT.EXECUTION.TRUTH, async (msg: any) => {
      if (this.reconciliationService) {
        await this.reconciliationService.handleTruthSnapshot(msg.payload || msg);
      }
    });
    ...
    // 5. Initialize and Start Regime Inference (P3: Regime Awareness)
    // We instantiate it here but it publishes to NATS. Brain also listens via NATS
    // to decouple the logic and allow RiskGuardian/Allocation to react to events.
    this.regimeInferenceService = new RegimeInferenceService(this.natsClient);
    await this.regimeInferenceService.start();

    // 6. Subscribe to Regime Changes (Update RiskGuardian)
    this.natsClient.subscribe(TITAN_SUBJECTS.EVT.BRAIN.REGIME, async (msg: any) => {
      const { regime } = msg.payload || msg;
      if (regime) {
        this.riskGuardian.setRegimeState(regime);
        logger.info(`[Brain] Applied Regime Update: ${regime}`);
      }
    });
  ...
  setExecutionEngine(client: ExecutionEngineClient): void {
    ...
    this.riskGuardian.setExecutionClient(client);
  ...
  setEquity(equity: number): void {
    ...
    this.stateManager.setEquity(safeEquity);
    this.updateComponents(safeEquity, this.stateManager.getPositions());
  }
  ...
  setDailyStartEquity(equity: number): void {
    ...
    this.circuitBreaker.setDailyStartEquity(equity);
  }

  private updateComponents(equity: number, positions: Position[]): void {
    this.riskGuardian.setEquity(equity);
    this.circuitBreaker.checkConditions({
      equity,
      positions,
      dailyStartEquity: this.stateManager.getDailyStartEquity(),
      recentTrades: [], // CircuitBreaker tracks trades internally via recordTrade (if wired) or we pass empty for equity check
    });
  }
  ...
  handleExecutionQualityUpdate(score: number): void {
    this.riskGuardian.updateExecutionQuality(score);
  }
  ...
  async handleOperatorAction(payload: any): Promise<void> {
    try {
      ...
      logger.info(`👨‍💻 Operator Action Received: ${action.type} (${action.reason})`);
      ...
      switch (action.type) {
        ...
        case 'DISARM_SYSTEM':
          ...
          break;
        case 'FLATTEN_ALL':
          logger.warn('⚠️ FLATTEN ALL COMMAND RECEIVED - Closing all positions');
  ...
  async verifyOperatorCredentials(
    ...
    const valid = await this.manualOverrideService.authenticateOperator(id, pass);
