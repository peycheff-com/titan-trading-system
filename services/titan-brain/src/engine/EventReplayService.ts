...
export class EventReplayService {
  ...
  async reconstructStateAt(timestamp: number): Promise<BrainStateManager> {
    this.logger.info(
      `🕰️ Time Travel: Reconstructing state at ${new Date(timestamp).toISOString()}`,
    );
    ...
    if (posSnap) {
      landmarks.push({
        ...
        apply: (sm) => {
          this.logger.debug(
            `   Applying Position Snapshot from ${new Date(Number(posSnap.timestamp)).toISOString()}`,
          );
    ...
    if (allocSnap) {
      landmarks.push({
        ...
        apply: (sm) => {
          this.logger.debug(
            `   Applying Allocation Snapshot from ${new Date(allocSnap.timestamp).toISOString()}`,
          );
    ...
    if (landmarks.length > 0) {
      ...
      this.logger.info(
        `   🚀 Jumping to snapshot at ${new Date(replayFrom).toISOString()} (${landmarks[0].type})`,
      );
    }
    ...
    while (replayFrom < timestamp) {
      ...
      if (
        remainingLandmarks[landmarkIndex] &&
        replayFrom === remainingLandmarks[landmarkIndex].timestamp
      ) {
        remainingLandmarks[landmarkIndex].apply(stateManager);
        landmarkIndex++;
        currentTarget = remainingLandmarks[landmarkIndex]
          ? remainingLandmarks[landmarkIndex].timestamp
          : timestamp;
      } else {
  ...
  private applyFill(fill: any, stateManager: BrainStateManager) {
    ...
    if (!pos) {
      ...
      if (
        ...
        pos.entryPrice =
          (pos.entryPrice * Math.abs(currentSignedSize) + fill.fillPrice * Math.abs(signedChange)) /
          totalSize;
      } else if (
