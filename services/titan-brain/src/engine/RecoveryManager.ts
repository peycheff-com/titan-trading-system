    ...
    if (this.stateRecoveryService) {
      ...
      if (state) {
        if (state.highWatermark) {
          this.capitalFlowManager.setHighWatermark(state.highWatermark);
        }
        ...
        if (state.positions && state.positions.length > 0) {
          ...
          if (currentPositions.length === 0) {
            state.positions.forEach((p) => this.stateManager.updatePosition(p));
            logger.info(`Recovered ${state.positions.length} positions`);
          }
        }
        ...
        if (state.dailyStartEquity !== undefined) {
          this.stateManager.setDailyStartEquity(state.dailyStartEquity);
        } else {
