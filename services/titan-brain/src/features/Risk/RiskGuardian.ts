...
/* eslint-disable functional/immutable-data, functional/no-let, @typescript-eslint/no-explicit-any -- RiskGuardian is stateful by design */
...
export class RiskGuardian {
  ...
  private readonly correlationCache: Map<string, CorrelationCacheEntry> =
    new Map();
  ...
  private updateAndPublishRiskState() {
    ...
    if (newState !== this.currentRiskState) {
      console.warn(
        `[RiskGuardian] State Transition: ${this.currentRiskState} -> ${newState}`,
      );
  ...
  async tripCircuitBreaker(reason: string) {
    ...
    if (
      ...
      typeof this.executionClient.haltSystem === "function"
    ) {
      try {
  ...
  private updateDynamicConfig(
    ...
    if (typeof features["risk.minStopDistanceMultiplier"] === "number") {
      this.config.minStopDistanceMultiplier =
        features["risk.minStopDistanceMultiplier"];
      updated = true;
    }
  ...
  setRegimeState(regime: RegimeState): void {
    if (this.currentRegime !== regime) {
      console.log(
        `[RiskGuardian] Regime updated: ${this.currentRegime} -> ${regime}`,
      );
  ...
  updateConfidence(driftDetected: boolean): void {
    ...
    if (driftDetected) {
      ...
      console.warn(
        `[RiskGuardian] Confidence degraded to ${
          this.confidenceScore.toFixed(3)
        } due to DRIFT`,
      );
    } else {
      // Slow recovery
  ...
  updateExecutionQuality(score: number): void {
    ...
    if (score < 0.7) {
      console.warn(
        `[RiskGuardian] Execution Quality degraded to ${score.toFixed(2)}`,
      );
      this.updateAndPublishRiskState();
    }
  }
  ...
  checkSignal(
    ...
    if (!this.checkTruthGate()) {
      return {
        approved: false,
        reason: `TRUTH_VETO: Confidence Score ${
          this.confidenceScore.toFixed(2)
        } < 0.8`,
        riskMetrics: this.getRiskMetrics(currentPositions),
      };
    }
    ...
    if (
      ...
      return {
        approved: false,
        reason:
          `Max Position Notional Exceeded: ${signal.requestedSize} > ${this.config.maxPositionNotional}`,
        riskMetrics: this.getRiskMetrics(currentPositions),
      };
    }
    ...
    if (this.executionQualityScore < 0.7) {
      return {
        approved: false,
        reason: `EXECUTION_QUALITY_VETO: Score ${
          this.executionQualityScore.toFixed(2)
        } < 0.7`,
        riskMetrics: this.getRiskMetrics(currentPositions),
      };
    }
      ...
      if (this.config.maxPositionNotional > 0) {
        ...
        if (projectedSize > this.config.maxPositionNotional) {
          return {
            approved: false,
            reason:
              `Max Position Notional Exceeded: ${projectedSize} > ${this.config.maxPositionNotional}`,
            riskMetrics,
          };
        }
      }
