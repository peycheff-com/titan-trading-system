...
// Common Schemas
export const TimestampSchema = z.number().int().positive().optional().default(
    () => Date.now(),
);
export const PhaseIdSchema = z.enum(["phase1", "phase2", "phase3"]);
export const SideSchema = z.enum(["BUY", "SELL"]);
export const DirectionSchema = z.enum(["LONG", "SHORT"]);
export const SymbolSchema = z.string().min(3).max(20).regex(/^[A-Z0-9]+$/);

// Signal Request Body
export const SignalRequestSchema = z.object({
    signalId: z.string().min(1).max(100).regex(/^[a-zA-Z0-9_-]+$/),
...
export const OverrideRequestSchema = z.object({
    operatorId: z.string().min(3).max(50).regex(/^[a-zA-Z0-9_-]+$/),
