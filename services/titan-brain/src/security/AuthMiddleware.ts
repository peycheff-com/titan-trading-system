...
export class AuthMiddleware {
    ...
    constructor(private readonly logger: Logger) {
        ...
        if (this.secret === "default-secret-change-me") {
            this.logger.warn(
                "⚠️ AuthMiddleware using default insecure secret!",
            );
        }
    }
    ...
    requireRole(requiredRole: string) {
        return async (request: FastifyRequest, reply: FastifyReply) => {
            ...
            if (
                !roles.includes(requiredRole) && !roles.includes("superadmin")
            ) {
                this.logger.warn(
                    `Access denied for user ${user.operatorId}. Required: ${requiredRole}, Has: ${
                        roles.join(",")
                    }`,
                );
