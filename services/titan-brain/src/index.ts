...
const INIT_NATS_TIMEOUT_MS = parsePositiveMs(process.env.TITAN_INIT_NATS_TIMEOUT_MS, 480000);
const STARTUP_MAX_TIME_MS = parsePositiveMs(process.env.TITAN_STARTUP_MAX_TIME_MS, 900000);
...
async function main(): Promise<void> {
  ...
  try {
    ...
    initSteps[3].execute = async () => {
      ...
      // Verify JetStream Integrity
      logger.info('   🔍 Verifying JetStream Integrity...');
      const jsm = nats.getJetStreamManager();
      if (!jsm) {
        throw new Error('Failed to get JetStream Manager from NatsClient');
      }
      const { verifyJetStreamConfiguration } = await import('./utils/checkJetStream.js');
      await verifyJetStreamConfiguration(jsm, logger as any);
      logger.info('   ✅ JetStream Integrity Verified');
    };
    ...
    initSteps[4].execute = async () => {
      ...
      eventReplayService = new EventReplayService(databaseManager!, logger as unknown as Logger);
    ...
    initSteps[5].execute = async () => {
      ...
      // Initialize Execution Quality Service
      const { ExecutionQualityService } = await import('./services/ExecutionQualityService.js');
      const executionQualityService = new ExecutionQualityService(getNatsClient());
      await executionQualityService.start();
      logger.info('   ✅ ExecutionQualityService initialized');

      // Register shutdown handler for Execution Quality Service
      startupManager!.registerShutdownHandler(async () => {
        await executionQualityService.stop(); // Assuming stop method exists or adding checking
      });
