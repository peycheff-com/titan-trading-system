...
import { getVenueStatusStore, resetVenueStatusStore } from '../services/venues/index.js';
...
export class WebhookServer {
  ...
  constructor(
    ...
    this.configRegistry = new ConfigRegistry(process.env.HMAC_SECRET);
    this.configController = new ConfigController(this.configRegistry, this.authMiddleware);
    this.venuesController = new VenuesController(this.logger, this.authMiddleware);
    ...
    this.operatorIntentService.hydrateFromDb().catch((err) => {
      this.logger.error(
        'Failed to hydrate intents from DB',
    ...
    if (!this.eventReplayService) {
      this.logger.warn(
        'EventReplayService not provided to WebhookServer. Historical state endpoints will fail.',
      );
    }
  ...
  async start(): Promise<void> {
    ...
    // Start venue status subscription if telemetry is enabled
    if (process.env.VENUES_TELEMETRY_LIVE === 'true' || process.env.VENUES_TELEMETRY_LIVE === '1') {
      getVenueStatusStore()
        .start()
        .catch((err) => {
          this.logger.warn(`VenueStatusStore failed to start: ${err.message}`);
        });
      this.logger.info('VenueStatusStore started for live venue telemetry');
    }
  ...
  private buildIntentExecutors(): Record<
    string,
    (intent: { type: string; params: Record<string, unknown> }) => Promise<{
      effect?: string;
  ...
  > {
    ...
    return {
      ARM: async () => {
        const prior = {
          armed: stateManager.isArmed(),
          posture: stateManager.isArmed() ? 'armed' : 'disarmed',
        };
        stateManager.setArmed(true);
        return {
          effect: 'System armed',
          prior_state: prior,
          new_state: { armed: true, posture: 'armed' },
        };
      },
      DISARM: async () => {
        const prior = {
        ...
        return {
          ...
          prior_state: prior,
          new_state: { armed: false, posture: 'disarmed' },
        };
      },
      ...
      THROTTLE_PHASE: async (intent) => {
        ...
        return {
          effect: `Phase ${phaseId} ${action}`,
          prior_state: { phase: phaseId },
          new_state: { phase: phaseId, action },
        };
      },
      RUN_RECONCILE: async () => {
        ...
        return {
          effect: 'Reconciliation service not available',
          error: 'NO_RECONCILIATION_SERVICE',
        };
      },
      FLATTEN: async () => {
      ...
      OVERRIDE_RISK: async (intent) => {
        ...
        setTimeout(() => {
          ...
          Logger.getInstance('webhook-server').info(
            `OVERRIDE_RISK auto-reverted after ${durationSec}s`,
          );
        }, durationSec * 1000);
  ...
  private buildIntentVerifiers(): Record<
    string,
    (intent: {
      type: string;
