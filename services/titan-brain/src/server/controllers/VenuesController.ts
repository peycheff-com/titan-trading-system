 ...
 */
const USE_LIVE_TELEMETRY =
  process.env.VENUES_TELEMETRY_LIVE === 'true' || process.env.VENUES_TELEMETRY_LIVE === '1';
...
export class VenuesController {
  ...
  constructor(logger: Logger, auth: AuthMiddleware) {
    this.logger = logger;
    this.auth = auth;
  }
  ...
  registerRoutes(server: FastifyInstance): void {
    ...
    server.get('/venues/summary', operatorGuard, this.handleGetSummary.bind(this));
    ...
    }>('/venues/:exchange/instruments', operatorGuard, this.handleGetInstruments.bind(this));
   ...
   */
  async handleGetSummary(_request: FastifyRequest, reply: FastifyReply): Promise<void> {
    try {
   ...
   */
  async handleGetVenues(_request: FastifyRequest, reply: FastifyReply): Promise<void> {
    try {
   ...
   */
  private async getExchangeStatus(exchangeId: string): Promise<ExchangeStatus | null> {
    ...
    const latency = connected ? Math.floor(Math.random() * 100) + 20 : undefined;
    ...
    return {
      ...
      connected,
      latency,
      lastHeartbeat: connected ? Date.now() - Math.floor(Math.random() * 30000) : undefined,
      products: {
        spot: info.spot,
      ...
      rateLimit: {
        remaining: connected ? Math.floor(Math.random() * 1000) + 500 : 0,
        limit: 1200,
  ...
  private getLiveExchangeStatus(
    ...
    return {
      ...
      lastHeartbeat: wsStatus.last_msg_ts ? new Date(wsStatus.last_msg_ts).getTime() : undefined,
      products: {
        spot: cached.status.capabilities.spot,
        futures: cached.status.capabilities.perps || cached.status.capabilities.futures,
  ...
  private async getInstruments(exchangeId: string): Promise<Instrument[]> {
    ...
    for (const symbol of symbols) {
      ...
      if (product === 'spot' && ['binance', 'bybit', 'mexc', 'kraken'].includes(exchangeId)) {
        instruments.push({
          symbol: exchangeId === 'kraken' ? `${base}USD.P` : `${base}USDT.P`,
          baseAsset: base,
          quoteAsset: quote,
          product: 'futures',
          status: 'trading',
  ...
  async handleShadowSimulate(
    request: FastifyRequest<{ Params: { exchange: string }; Body: ShadowOrderRequest }>,
    reply: FastifyReply,
  ): Promise<void> {
    try {
      const { exchange } = request.params;
  ...
  private async simulateOrder(
    ...
    if (order.quantity > 10) {
      warnings.push('Large position size - may trigger additional margin');
      ...
      estimatedFill:
        errors.length === 0
          ? {
              ...
              fee,
              total: mockPrice * order.quantity + (order.side === 'buy' ? fee : -fee),
            }
          : undefined,
      riskChecks,
    };
