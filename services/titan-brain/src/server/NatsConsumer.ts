...
export class NatsConsumer {
  ...
  private subscribeToTopics(): void {
    ...
    this.nats.subscribe(
      "titan.evt.quality.execution", // TODO: Use constant from TITAN_SUBJECTS or similar when available
      async (data: any, subject) => {
        try {
          if (
            data && data.score && typeof data.score.total_score === "number"
          ) {
            await this.brain.handleExecutionQualityUpdate(
              data.score.total_score,
            );
          }
        } catch (err) {
          this.logger.error(
            "Error handling Execution Quality Update",
            err as Error,
          );
        }
      },
