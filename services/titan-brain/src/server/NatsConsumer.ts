...
export class NatsConsumer {
  ...
  private subscribeToTopics(): void {
    ...
    // Subscribe to Execution Quality Updates
    this.nats.subscribe(
      TitanSubject.EVT_QUALITY_EXECUTION,
      async (data: any, subject) => {
        try {
          if (data && data.score && typeof data.score.total_score === 'number') {
            await this.brain.handleExecutionQualityUpdate(data.score.total_score);
          }
        } catch (err) {
          this.logger.error('Error handling Execution Quality Update', err as Error);
        }
      },
      'BRAIN_QUALITY_GATE', // Durable consumer for quality gating
    );
  }
