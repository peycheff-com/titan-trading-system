...
import { CircuitBreaker, CircuitBreakerDefaults } from '../../src/services/CircuitBreaker';
...
describe('Service Communication Integration Tests', () => {
  ...
  beforeEach(() => {
    ...
    circuitBreaker = new CircuitBreaker('test-service', CircuitBreakerDefaults.IMPORTANT);
  ...
  describe('ServiceDiscovery integration', () => {
    it('should register and discover services', () => {
      ...
      expect(serviceDiscovery.getServiceUrl('phase1-service')).toBe('http://localhost:3001');
      expect(serviceDiscovery.getServiceUrl('phase2-service')).toBe('http://localhost:3002');
    ...
    it('should handle service health monitoring', async () => {
      ...
      const healthyStatus = serviceDiscovery.getServiceStatus('healthy-service');
      const unhealthyStatus = serviceDiscovery.getServiceStatus('unhealthy-service');
  ...
  describe('End-to-end service communication', () => {
    ...
    it('should handle service discovery with health checks', async () => {
      ...
      const primaryStatus = serviceDiscovery.getServiceStatus('primary-service');
  ...
  describe('Error handling and resilience', () => {
    it('should handle network timeouts gracefully', async () => {
      const timeoutClient = new ServiceClient('timeout-service', 'http://localhost:3000', {
        timeout: 100, // Very short timeout
        retries: 1,
        circuitBreaker: new CircuitBreaker('timeout-service', CircuitBreakerDefaults.IMPORTANT)
      });
