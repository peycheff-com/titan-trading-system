...
export class ShippingGate {
  ...
  evaluate(baseline: BacktestResult, proposed: BacktestResult): ValidationReport {
    ...
    if (proposed.maxDrawdownPercent > this.config.maxDrawdown) {
      ...
      report.passed = false;
      // eslint-disable-next-line functional/immutable-data
      report.rejectionReason = `Max Drawdown ${(proposed.maxDrawdownPercent * 100).toFixed(
        2,
    ...
    if (proposed.maxDrawdownPercent > baseline.maxDrawdownPercent * 1.1) {
      // eslint-disable-next-line functional/immutable-data
      report.passed = false;
      // eslint-disable-next-line functional/immutable-data
    ...
    if (proposed.sharpeRatio < this.config.minSharpe) {
      ...
      report.passed = false;
      // eslint-disable-next-line functional/immutable-data
      report.rejectionReason = `Sharpe Ratio ${proposed.sharpeRatio.toFixed(
        2,
