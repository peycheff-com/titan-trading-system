...
export class HistoricalDataService {
  ...
  async getRegimeSnapshots(symbol: string, start: number, end: number): Promise<RegimeSnapshot[]> {
    const client = await this.db.connect();
    try {
      ...
      this.logger.warn(`Failed to fetch regime snapshots for ${symbol}`, String(error));
  ...
  private validateContinuity(candles: OHLCV[], timeframe: string) {
    ...
    for (let i = 1; i < candles.length; i++) {
      ...
      if (diff > intervalMs * 1.5) {
        // Allow slight jitter, but flag big gaps
        this.logger.warn(`Data Gap detected for ${candles[i].symbol} ${timeframe}`, undefined, {
