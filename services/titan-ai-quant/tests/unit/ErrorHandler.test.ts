...
describe("ErrorHandler", () => {
  ...
  describe("getUserFriendlyMessage", () => {
    it("should return user-friendly message for each error code", () => {
      expect(getUserFriendlyMessage(ErrorCode.RATE_LIMIT)).toContain(
        "temporarily busy",
      );
      ...
      expect(getUserFriendlyMessage(ErrorCode.CONFIG_PARSE_ERROR)).toContain(
        "invalid format",
      );
      expect(getUserFriendlyMessage(ErrorCode.UNKNOWN_COMMAND)).toContain(
        "/help",
      );
    });
  ...
  describe("withRetry", () => {
    ...
    it("should throw after max retries", async () => {
      const fn = jest.fn<() => Promise<any>>().mockRejectedValue(
    ...
    it("should use custom shouldRetry function", async () => {
      ...
      const shouldRetry = jest.fn<(error: any) => boolean>().mockReturnValue(
        true,
      );
