...
describe('RealTimeOptimizer', () => {
  ...
  describe('data handling', () => {
    it('should handle signal events', () => {
      ...
      expect(optimizer.getStats().dataStreamStats.lastUpdate).toBeGreaterThan(0);
    });
  ...
  describe('error handling', () => {
    ...
    it('should handle proposal processing errors', async () => {
      ...
      mockAnalyst.validateProposal.mockRejectedValue(new Error('Validation error'));

      // Should not throw error
      await expect(optimizer['processOptimizationProposal'](proposal, [])).resolves.not.toThrow();
    });
  });
});