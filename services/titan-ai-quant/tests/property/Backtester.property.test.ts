...
import { Trade, OHLCV, RegimeSnapshot, Config, BacktestResult } from '../../src/types';
...
describe('Backtester Property Tests', () => {
    ...
    fc.assert(
      fc.property(
        ...
        fc.record({
          ...
          totalPnL: fc.float({ min: Math.fround(-10000), max: Math.fround(10000), noNaN: true }),
          avgPnL: fc.float({ min: Math.fround(-1000), max: Math.fround(1000), noNaN: true }),
          maxDrawdown: fc.float({ min: Math.fround(0), max: Math.fround(5000), noNaN: true }),
          maxDrawdownPercent: fc.float({ min: Math.fround(0), max: Math.fround(0.5), noNaN: true }),
          sharpeRatio: fc.float({ min: Math.fround(-5), max: Math.fround(5), noNaN: true }),
          avgSlippage: fc.float({ min: Math.fround(0), max: Math.fround(1), noNaN: true }),
        ...
        (baselineResult, proposedResult) => {
          ...
          const report = backtester.createValidationReport(baselineResult, proposedResult, backtestPeriod);
          ...
          expect(report.deltas.pnlDelta).toBeCloseTo(proposedResult.totalPnL - baselineResult.totalPnL, 2);
          expect(report.deltas.drawdownDelta).toBeCloseTo(proposedResult.maxDrawdown - baselineResult.maxDrawdown, 2);
          expect(report.deltas.winRateDelta).toBeCloseTo(proposedResult.winRate - baselineResult.winRate, 5);
          ...
          if (Math.abs(baselineResult.totalPnL) > 0.01) { // Avoid division by very small numbers
            const expectedPnlDeltaPercent = (report.deltas.pnlDelta / Math.abs(baselineResult.totalPnL)) * 100;
            expect(report.deltas.pnlDeltaPercent).toBeCloseTo(expectedPnlDeltaPercent, 2);
          }
  ...
  it('should load historical data consistently for any valid input', async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.array(fc.string({ minLength: 3, maxLength: 10 }).filter(s => s.trim().length > 0), { minLength: 1, maxLength: 5 }),
        ...
        async (symbols, startTime, endTime) => {
          ...
          validSymbols.forEach(symbol => {
            const ohlcvData: OHLCV[] = [
              { timestamp: startTime + 100, open: 100, high: 105, low: 95, close: 102, volume: 1000 },
              { timestamp: startTime + 200, open: 102, high: 108, low: 100, close: 106, volume: 1200 },
            ];
          ...
          const result = await backtester.loadHistoricalData(symbols, startTime, endTime);
        ...
        fc.record({
          traps: fc.record({
            oi_wipeout: fc.record({
              enabled: fc.boolean(),
              stop_loss: fc.float({ min: Math.fround(0.01), max: Math.fround(0.1), noNaN: true }),
              take_profit: fc.float({ min: Math.fround(0.01), max: Math.fround(0.2), noNaN: true }),
              risk_per_trade: fc.float({ min: Math.fround(0.001), max: Math.fround(0.05), noNaN: true }),
              max_leverage: fc.integer({ min: 1, max: 100 }),
              min_confidence: fc.float({ min: Math.fround(0.1), max: Math.fround(1), noNaN: true }),
              cooldown_period: fc.integer({ min: 60, max: 3600 })
            })
          }),
          risk: fc.record({
            max_daily_loss: fc.float({ min: Math.fround(0.01), max: Math.fround(0.2), noNaN: true }),
            max_position_size: fc.float({ min: Math.fround(0.1), max: Math.fround(1), noNaN: true }),
            max_open_positions: fc.integer({ min: 1, max: 10 }),
            emergency_flatten_threshold: fc.float({ min: Math.fround(0.05), max: Math.fround(0.5), noNaN: true })
          }),
          execution: fc.record({
            latency_penalty: fc.integer({ min: 50, max: 500 }),
            slippage_model: fc.constantFrom('conservative' as const, 'realistic' as const, 'optimistic' as const),
      ...
      fc.asyncProperty(
        fc.array(
          fc.record({
            ...
            trapType: fc.constantFrom('oi_wipeout' as const, 'funding_spike' as const, 'liquidity_sweep' as const, 'volatility_spike' as const),
            side: fc.constantFrom('long' as const, 'short' as const),
            entryPrice: fc.float({ min: Math.fround(100), max: Math.fround(200), noNaN: true }),
            exitPrice: fc.float({ min: Math.fround(100), max: Math.fround(200), noNaN: true }),
            ...
            pnlPercent: fc.float({ min: Math.fround(-50), max: Math.fround(50), noNaN: true }),
            duration: fc.integer({ min: 10000, max: 300000 }),
            slippage: fc.float({ min: Math.fround(0), max: Math.fround(0.5), noNaN: true }),
            fees: fc.float({ min: Math.fround(0), max: Math.fround(10), noNaN: true }),
            exitReason: fc.constantFrom('take_profit' as const, 'stop_loss' as const, 'timeout' as const)
          }),
        ...
        async (trades, startTime, endTime) => {
          const config: Config = {
            traps: {
              oi_wipeout: { enabled: true, stop_loss: 0.02, take_profit: 0.05, risk_per_trade: 0.01, max_leverage: 20, min_confidence: 0.7, cooldown_period: 300 },
              funding_spike: { enabled: true, stop_loss: 0.02, take_profit: 0.05, risk_per_trade: 0.01, max_leverage: 20, min_confidence: 0.7, cooldown_period: 300 },
              liquidity_sweep: { enabled: false, stop_loss: 0.02, take_profit: 0.05, risk_per_trade: 0.01, max_leverage: 20, min_confidence: 0.7, cooldown_period: 300 },
              volatility_spike: { enabled: false, stop_loss: 0.02, take_profit: 0.05, risk_per_trade: 0.01, max_leverage: 20, min_confidence: 0.7, cooldown_period: 300 },
            },
            risk: {
              max_daily_loss: 0.05,
          ...
          if (expectedTrades.length > 0) {
            ...
            expect(result.warnings.some(w => w.code === 'MISSING_OHLCV_DATA' || w.code === 'INCOMPLETE_REGIME_DATA')).toBe(true);
          }
        }
      ),
      { numRuns: 30 }
      ...
      fc.asyncProperty(
        fc.record({
          traps: fc.record({
            oi_wipeout: fc.record({
              enabled: fc.boolean(),
              stop_loss: fc.float({ min: Math.fround(0.001), max: Math.fround(0.2), noNaN: true }),
              take_profit: fc.float({ min: Math.fround(0.001), max: Math.fround(0.5), noNaN: true }),
              risk_per_trade: fc.float({ min: Math.fround(0.001), max: Math.fround(0.1), noNaN: true }),
              max_leverage: fc.integer({ min: 1, max: 100 }),
              min_confidence: fc.float({ min: Math.fround(0.1), max: Math.fround(1), noNaN: true }),
          ...
          risk: fc.record({
            max_daily_loss: fc.float({ min: Math.fround(0.01), max: Math.fround(0.5), noNaN: true }),
            max_position_size: fc.float({ min: Math.fround(0.01), max: Math.fround(1), noNaN: true }),
            max_open_positions: fc.integer({ min: 1, max: 20 }),
            emergency_flatten_threshold: fc.float({ min: Math.fround(0.01), max: Math.fround(1), noNaN: true })
          }),
          execution: fc.record({
