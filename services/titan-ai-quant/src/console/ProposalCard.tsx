...
export function ProposalCard({
  ...
  const riskColor =
    expectedImpact.riskChange <= 0 ? 'green' : expectedImpact.riskChange <= 5 ? 'yellow' : 'red';
  const confidenceColor =
    expectedImpact.confidenceScore >= 0.7
      ? 'green'
      : expectedImpact.confidenceScore >= 0.5
        ? 'yellow'
        : 'red';
  ...
  return (
    <Box flexDirection="column" borderStyle="single" borderColor="cyan" padding={1}>
      ...
      <Box justifyContent="space-between">
        ...
        {status && <Text color={statusColor}>[{status.toUpperCase()}]</Text>}
      </Box>
        ...
        <Box marginTop={1} flexDirection="column">
          ...
          <Box marginLeft={2} flexDirection="column">
            ...
            <Box>
              ...
              <Text dimColor>
                {' '}
                ({proposal.validationReport.deltas.pnlDeltaPercent >= 0 ? '+' : ''}
                {proposal.validationReport.deltas.pnlDeltaPercent.toFixed(1)}%)
              </Text>
            </Box>
            <Box>
              ...
              <Text
                color={
                  proposal.validationReport.recommendation === 'approve'
                    ? 'green'
                    : proposal.validationReport.recommendation === 'reject'
                      ? 'red'
                      : 'yellow'
                }
              >
