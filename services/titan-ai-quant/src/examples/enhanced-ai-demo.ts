...
import { EnhancedAIIntegration } from '../ai/EnhancedAIIntegration';
import { TitanAnalyst } from '../ai/TitanAnalyst';
import { OHLCV, Trade, RegimeSnapshot } from '../types';
...
async function runEnhancedAIDemo(): Promise<void> {
  ...
  aiIntegration.on('parameterOptimized', (event) => {
    ...
    console.log(`   ${event.proposal.currentValue} → ${event.proposal.suggestedValue}`);
  ...
  aiIntegration.on('strategySelectionUpdated', (event) => {
    ...
    event.selection.selectedStrategies.forEach(strategy => {
      console.log(`   ${strategy.strategy}: ${(strategy.allocation * 100).toFixed(1)}% (conf: ${(strategy.confidence * 100).toFixed(0)}%)`);
    });
    console.log();
  });

  aiIntegration.on('riskAdjusted', (event) => {
    console.log(`⚠️ Risk adjustment applied (score: ${event.riskScore.toFixed(1)}/100):`);
    event.adjustments.forEach(adj => {
      console.log(`   ${adj.trigger}: ${adj.currentRisk.toFixed(3)} → ${adj.recommendedRisk.toFixed(3)} (${adj.urgency})`);
    });
    console.log();
  });
  ...
  aiIntegration.on('abTestCompleted', (event) => {
    ...
    console.log(`   Confidence: ${(event.result.confidence * 100).toFixed(1)}%\n`);
  });
  ...
  for (const symbol of symbols) {
    ...
    for (let i = 0; i < 50; i++) {
      const basePrice = symbol === 'BTCUSDT' ? 50000 : symbol === 'ETHUSDT' ? 3000 : 1;
  ...
  for (let i = 0; i < 30; i++) {
    ...
    const trapType = ['oi_wipeout', 'funding_spike', 'liquidity_sweep'][Math.floor(Math.random() * 3)] as any;
  ...
  const statusInterval = setInterval(() => {
    ...
    console.log(`📈 Performance Score: ${status.performanceScore.toFixed(1)}/100 | Risk Level: ${status.riskLevel.toUpperCase()}`);
    console.log(`🔄 Optimizations: ${status.realTimeOptimizer.optimizationCount} | A/B Tests: ${status.realTimeOptimizer.activeABTests}`);
    console.log(`📊 Symbols Tracked: ${status.predictiveAnalytics.symbolsTracked} | Models Active: ${status.predictiveAnalytics.modelsActive}`);
    console.log('---');
  }, 10000);
