...
import { RegimeSnapshot, OHLCV, Trade, Config } from '../types';

/**
 * Market regime types
 */
export type MarketRegime = 'bull_trending' | 'bear_trending' | 'sideways' | 'high_volatility' | 'low_volatility' | 'risk_off' | 'risk_on';
...
export interface RiskAdjustment {
  timestamp: number;
  trigger: 'volatility_spike' | 'correlation_increase' | 'regime_change' | 'performance_degradation';
  currentRisk: number;
...
export class PredictiveAnalytics extends EventEmitter {
  ...
  addRegimeSnapshot(snapshot: RegimeSnapshot): void {
    ...
    this.emit('regimeUpdated', { symbol, regime: this.currentRegimes.get(symbol) });
  }
  ...
  detectMarketRegime(symbol: string): MarketRegime | null {
    ...
    if (volatility > 0.03) { // High volatility threshold
      ...
      const avgRegimeState = recentRegimes.reduce((sum, r) => sum + r.regimeState, 0) / recentRegimes.length;
  ...
  predictStrategyPerformance(strategy: string, symbol: string): StrategyPrediction | null {
    ...
    const winProbability = historicalTrades.filter(t => t.pnl > 0).length / historicalTrades.length;
  ...
  generateRiskAdjustment(currentConfig: Config): RiskAdjustment[] {
    ...
    if (correlationAnalysis.portfolioCorrelation > 0.8) {
      adjustments.push({
        ...
        adjustment: -0.3,
        reasoning: 'High portfolio correlation detected, reducing position sizes to manage concentration risk',
        confidence: 0.85,
        urgency: 'high'
      });
    }
    ...
    for (const symbol of this.marketData.keys()) {
      ...
      if (volatilityPrediction && volatilityPrediction.predictedVolatility > volatilityPrediction.currentVolatility * 1.5) {
        adjustments.push({
          timestamp: Date.now(),
   ...
   */
  private calculateCorrelation(symbol1: string, symbol2: string): { correlation: number; significance: number } | null {
    const data1 = this.marketData.get(symbol1);
    const data2 = this.marketData.get(symbol2);
    
    if (!data1 || !data2 || data1.length < this.config.correlationWindow || data2.length < this.config.correlationWindow) {
      return null;
    }
  ...
  private getRegimeAdjustment(regime: MarketRegime): {
    ...
    switch (regime) {
      case 'bull_trending':
        return { returnMultiplier: 1.2, volatilityMultiplier: 0.9, winRateMultiplier: 1.1, drawdownMultiplier: 0.8, sharpeMultiplier: 1.3 };
      case 'bear_trending':
        return { returnMultiplier: 0.8, volatilityMultiplier: 1.2, winRateMultiplier: 0.9, drawdownMultiplier: 1.3, sharpeMultiplier: 0.7 };
      case 'high_volatility':
        return { returnMultiplier: 1.1, volatilityMultiplier: 1.5, winRateMultiplier: 0.8, drawdownMultiplier: 1.5, sharpeMultiplier: 0.6 };
      case 'low_volatility':
        return { returnMultiplier: 0.9, volatilityMultiplier: 0.6, winRateMultiplier: 1.1, drawdownMultiplier: 0.7, sharpeMultiplier: 1.2 };
      case 'risk_off':
        return { returnMultiplier: 0.7, volatilityMultiplier: 1.3, winRateMultiplier: 0.8, drawdownMultiplier: 1.4, sharpeMultiplier: 0.5 };
      case 'risk_on':
        return { returnMultiplier: 1.1, volatilityMultiplier: 1.1, winRateMultiplier: 1.0, drawdownMultiplier: 1.0, sharpeMultiplier: 1.0 };
      default: // sideways
        return { returnMultiplier: 1.0, volatilityMultiplier: 1.0, winRateMultiplier: 1.0, drawdownMultiplier: 1.0, sharpeMultiplier: 1.0 };
    }
  }
   ...
   */
  private getRecommendedAction(expectedReturn: number, regime: MarketRegime): 'increase' | 'decrease' | 'maintain' | 'pause' {
    if (regime === 'risk_off' || regime === 'high_volatility') {
