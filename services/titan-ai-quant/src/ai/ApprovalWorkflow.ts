...
import { StrategicMemory } from './StrategicMemory';
import type { OptimizationProposal, Config } from '../types/index';
import { safeValidateConfig } from '../config/ConfigSchema';
...
export class ApprovalWorkflow {
    ...
    try {
      ...
      if (!validationResult.success) {
        const error = new TitanError(
          ...
          { targetKey: proposal.targetKey, suggestedValue: proposal.suggestedValue }
        );
        this.handleError(error);
        return { 
          success: false, 
          error: getUserFriendlyMessage(error.code, validationResult.error.message)
        };
      }
      ...
      this.saveConfig(validationResult.data);
      ...
      try {
        this.onConfigUpdate?.(validationResult.data);
        // Update last known good config on successful hot reload
        this.lastKnownGoodConfig = validationResult.data;
      } catch (hotReloadError) {
        // Hot reload failed - rollback to previous config
        const error = new TitanError(
          ErrorCode.HOT_RELOAD_FAILURE,
          `Hot reload failed: ${hotReloadError instanceof Error ? hotReloadError.message : String(hotReloadError)}`,
      ...
      const titanError = error instanceof TitanError 
        ? error 
        : new TitanError(
  ...
  async rejectProposal(
    ...
    try {
      ...
      await this.memory.storeInsight(
        ...
        `Suggested change: ${JSON.stringify(proposal.currentValue)} → ${JSON.stringify(proposal.suggestedValue)}`,
  ...
  async rollbackConfig(versionTag: string): Promise<ApprovalResult> {
    ...
    try {
      ...
      if (!configVersion) {
        return { success: false, error: `Config version ${versionTag} not found` };
      }
      ...
      if (!validationResult.success) {
        return { 
          success: false, 
          error: `Stored config validation failed: ${validationResult.error.message}` 
        };
      }
