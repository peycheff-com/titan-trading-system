...
import { DataLoader } from '../simulation/DataLoader';
import { Trade, RegimeSnapshot, OHLCV, Config, OptimizationProposal, ValidationReport, Insight } from '../types';
import { TitanError, ErrorCode, logError } from '../utils/ErrorHandler';
...
export class OptimizationWorkflow {
  ...
  private async loadHistoricalData(): Promise<{
    ...
    try {
      ...
      throw new TitanError(
        ...
        { startTime, endTime, error: error instanceof Error ? error.message : 'Unknown error' }
      );
    }
  }
  ...
  private async processProposals(
    insights: Insight[],
    historicalData: { trades: Trade[]; ohlcvData: OHLCV[]; regimeSnapshots: RegimeSnapshot[] }
  ): Promise<Array<{
    proposal: OptimizationProposal;
    validation: ValidationReport;
    applied: boolean;
    ...
    for (const insight of limitedInsights) {
      try {
        ...
        console.log(`Validation result: ${validation.recommendation} (confidence: ${validation.confidenceScore.toFixed(2)})`);
  ...
  private async applyApprovedProposals(
    ...
    for (const result of proposalResults) {
      ...
      if (shouldAutoApply) {
        ...
        console.log(`Proposal requires manual review: ${proposal.targetKey} (${validation.recommendation}, confidence: ${validation.confidenceScore.toFixed(2)})`);
      }
    }
