...
import { MorningBriefing, Insight, OptimizationProposal, ValidationReport, Config, Trade, RegimeSnapshot } from '../types';
import { Journal } from '../ai/Journal';
import { TitanAnalyst } from '../ai/TitanAnalyst';
import { StrategicMemory } from '../ai/StrategicMemory';
import { Backtester, InMemoryDataCache } from '../simulation/Backtester';
...
export class NightlyOptimize {
  ...
  constructor(config: NightlyOptimizeConfig = {}) {
    this.config = {
      ...
      tradesFilePath: config.tradesFilePath ?? path.join(process.cwd(), 'logs', 'trades.jsonl'),
      regimeFilePath: config.regimeFilePath ?? path.join(process.cwd(), 'logs', 'regime_snapshots.jsonl'),
      configFilePath: config.configFilePath ?? path.join(process.cwd(), 'config', 'phase1.config.json'),
      briefingFilePath: config.briefingFilePath ?? path.join(process.cwd(), 'logs', 'morning_briefing.json'),
      dbPath: config.dbPath ?? path.join(process.cwd(), 'data', 'strategic_memory.db'),
      minConfidence: config.minConfidence ?? 0.7,
      maxProposals: config.maxProposals ?? 3,
    };

    this.journal = new Journal(this.config.tradesFilePath, this.config.regimeFilePath);
  ...
  async runOptimization(): Promise<MorningBriefing> {
    ...
    const proposals: Array<{ proposal: OptimizationProposal; validation: ValidationReport }> = [];
  ...
  generateBriefing(
    ...
    if (proposals.length > 0) {
      summaryParts.push(`generated ${proposals.length} optimization proposal${proposals.length !== 1 ? 's' : ''}`);
    }
   ...
   * Calculate performance summary from trades
   */
  private calculatePerformanceSummary(trades: Trade[]): MorningBriefing['performanceSummary'] {
    if (trades.length === 0) {
  ...
  static loadBriefing(briefingFilePath?: string): MorningBriefing | null {
    const filePath = briefingFilePath ?? path.join(process.cwd(), 'logs', 'morning_briefing.json');
