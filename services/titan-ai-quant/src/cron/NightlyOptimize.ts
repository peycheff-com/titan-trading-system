...
export class NightlyOptimize {
  ...
  constructor(config: NightlyOptimizeConfig = {}) {
    this.config = {
      ...
      regimeFilePath:
        config.regimeFilePath ?? path.join(process.cwd(), 'logs', 'regime_snapshots.jsonl'),
      configFilePath:
        config.configFilePath ?? path.join(process.cwd(), 'config', 'phase1.config.json'),
      briefingFilePath:
        config.briefingFilePath ?? path.join(process.cwd(), 'logs', 'morning_briefing.json'),
