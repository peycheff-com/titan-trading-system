        ...
        <TableBody>
          {logs.map((log) => (
            <TableRow key={log.id}>
              ...
              <TableCell>
                 <span className={cn("text-xs font-bold", log.side === 'LONG' ? "text-emerald-500" : "text-red-500")}>
                    {log.side}
                 </span>
              </TableCell>
              ...
              <TableCell>
                 <Badge variant={log.status === 'ACCEPTED' ? 'default' : 'secondary'} className={cn("text-[10px]", log.status === 'REJECTED' && "bg-muted text-muted-foreground")}>
                    {log.status}
                 </Badge>
              </TableCell>
