...
const Settings = () => {
  ...
  if (loading) {
    return <div className="p-8 flex justify-center text-muted-foreground">Loading Configuration...</div>;
  }

  if (!config || !localRisk || !localWhitelist) {
    return <div className="p-8 flex justify-center text-destructive">Failed to load configuration. Check backend connection.</div>;
  }
  ...
  return (
    <div className="container mx-auto p-6 max-w-5xl space-y-8">
      <div>
        ...
        <p className="text-muted-foreground">Manage your Titan Execution configuration, risk parameters, and connections.</p>
      </div>
      ...
      <Tabs defaultValue="general" className="w-full">
        <TabsList className="grid w-full grid-cols-6 lg:w-[600px]">
          ...
          <TabsTrigger value="connections" className="text-blue-400">Connections</TabsTrigger>
        </TabsList>
              ...
              <div className="flex items-center justify-between rounded-lg border p-4">
                  <div className="space-y-0.5">
                    ...
                    <div className="text-sm text-muted-foreground">
                      Current mode: <span className="font-mono font-semibold">{config.mode || 'UNKNOWN'}</span>
                    </div>
                  </div>
        ...
        <TabsContent value="risk">
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2"><Shield className="h-5 w-5" /> Risk Management</CardTitle>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-6">
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between">
                      ...
                      <span className="font-mono text-sm">{(localRisk.phase1_risk_pct * 100).toFixed(1)}%</span>
                    </div>
                    <Slider 
                      value={[localRisk.phase1_risk_pct * 100]} 
                      min={1} max={20} step={0.5}
                      onValueChange={(val) => setLocalRisk({...localRisk, phase1_risk_pct: val[0] / 100})}
                    />
                  </div>
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between">
                      ...
                      <span className="font-mono text-sm">{(localRisk.phase2_risk_pct * 100).toFixed(1)}%</span>
                    </div>
                    <Slider 
                      value={[localRisk.phase2_risk_pct * 100]} 
                      min={1} max={10} step={0.5}
                      onValueChange={(val) => setLocalRisk({...localRisk, phase2_risk_pct: val[0] / 100})}
                    />
                  </div>
                </div>
                ...
                {localGuardrails && (
                  <div className="space-y-6 border-l pl-6">
                    <h3 className="font-semibold text-sm uppercase text-muted-foreground">Guardrails (Hard Limits)</h3>
                    
                    <div className="space-y-2">
                       <Label>Max Leverage (x)</Label>
                       <Input type="number" value={localGuardrails.maxLeverage} onChange={(e) => setLocalGuardrails({...localGuardrails, maxLeverage: parseInt(e.target.value) || 1})} />
                    </div>
                    <div className="space-y-2">
                       <Label>Max Position Size %</Label>
                       <Input type="number" value={localGuardrails.maxPositionSizePct} onChange={(e) => setLocalGuardrails({...localGuardrails, maxPositionSizePct: parseInt(e.target.value) || 1})} />
                    </div>
                     <div className="space-y-2">
                       <Label>Min Confidence Score (0-1)</Label>
                       <Input type="number" step="0.1" value={localGuardrails.minConfidenceScore} onChange={(e) => setLocalGuardrails({...localGuardrails, minConfidenceScore: parseFloat(e.target.value) || 0.1})} />
                    </div>
                  </div>
                )}
              </div>
            ...
            <CardFooter className="flex justify-between">
              <Button onClick={handleRiskSave} disabled={saving}><Save className="mr-2 h-4 w-4"/> Save Risk Phases</Button>
              <Button onClick={handleGuardrailsSave} variant="secondary" disabled={saving}><Save className="mr-2 h-4 w-4"/> Save Guardrails</Button>
            </CardFooter>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="fees">
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2"><Wallet className="h-5 w-5" /> Trading Fees</CardTitle>
              <CardDescription>Configure Exchange Fees (affects limit chaser thresholds).</CardDescription>
            </CardHeader>
                ...
                <>
                  <div className="space-y-4">
                    <div className="flex justify-between">
                      ...
                      <span className="font-mono text-sm">{(localFees.maker_fee_pct * 100).toFixed(4)}%</span>
                    </div>
                    <Slider 
                      value={[localFees.maker_fee_pct * 100]} 
                      min={0} max={0.2} step={0.001}
                      onValueChange={(val) => setLocalFees({...localFees, maker_fee_pct: val[0] / 100})}
                    />
                  </div>
                  <div className="space-y-4">
                     <div className="flex justify-between">
                      <Label>Taker Fee %</Label>
                      <span className="font-mono text-sm">{(localFees.taker_fee_pct * 100).toFixed(4)}%</span>
                    </div>
                    <Slider 
                      value={[localFees.taker_fee_pct * 100]} 
                      min={0} max={0.2} step={0.001}
                      onValueChange={(val) => setLocalFees({...localFees, taker_fee_pct: val[0] / 100})}
                    />
                  </div>
                </>
              )}
            ...
            <CardFooter>
              <Button onClick={handleFeesSave} disabled={saving}><Save className="mr-2 h-4 w-4"/> Save Fees</Button>
            </CardFooter>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="safety">
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2"><Shield className="h-5 w-5" /> Safety Gates</CardTitle>
            ...
            <CardContent className="space-y-6">
                ...
                <>
                   <div className="space-y-2">
                    ...
                    <Input 
                      ...
                      onChange={(e) => setLocalSafety({...localSafety, max_consecutive_losses: parseInt(e.target.value) || 0})}
                    />
                    <p className="text-xs text-muted-foreground">Trigger circuit breaker after N consecutive losses.</p>
                  </div>

                  <div className="space-y-4">
                    <div className="flex justify-between">
                      <Label>Max Daily Drawdown %</Label>
                      <span className="font-mono text-sm">{(localSafety.max_daily_drawdown_pct * 100).toFixed(1)}%</span>
                    </div>
                    <Slider 
                      value={[localSafety.max_daily_drawdown_pct * 100]} 
                      min={1} max={20} step={0.5}
                      onValueChange={(val) => setLocalSafety({...localSafety, max_daily_drawdown_pct: val[0] / 100})}
                    />
                  </div>
                   ...
                   <div className="space-y-4">
                    <div className="flex justify-between">
                      ...
                      <span className="font-mono text-sm">{(localSafety.max_weekly_drawdown_pct * 100).toFixed(1)}%</span>
                    </div>
                    <Slider 
                      value={[localSafety.max_weekly_drawdown_pct * 100]} 
                      min={1} max={30} step={0.5}
                      onValueChange={(val) => setLocalSafety({...localSafety, max_weekly_drawdown_pct: val[0] / 100})}
                    />
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input 
                      ...
                      onChange={(e) => setLocalSafety({...localSafety, circuit_breaker_cooldown_hours: parseInt(e.target.value) || 0})}
                    />
                  </div>
                </>
              )}
             ...
             <CardFooter>
              <Button onClick={handleSafetySave} disabled={saving}><Save className="mr-2 h-4 w-4"/> Save Safety Settings</Button>
            </CardFooter>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="backtester">
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2"><Activity className="h-5 w-5" /> Simulation & Backtesting</CardTitle>
              <CardDescription>Configure "Bulgaria Mode" parameters for realistic simulation.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              {localBacktester && (
                <>
                  <div className="space-y-4">
                     <div className="flex justify-between">
                      <Label>Simulated Latency (ms)</Label>
                       <span className="font-mono text-sm">{localBacktester.bulgariaLatencyMs}ms</span>
                    </div>
                    <Slider 
                      value={[localBacktester.bulgariaLatencyMs]} 
                      min={0} max={2000} step={10}
                      onValueChange={(val) => setLocalBacktester({...localBacktester, bulgariaLatencyMs: val[0]})}
                    />
                    <p className="text-xs text-muted-foreground">Artificial delay added to order execution in backtests.</p>
                  </div>

                  <div className="space-y-4">
                     <div className="flex justify-between">
                      <Label>Simulated Slippage %</Label>
                       <span className="font-mono text-sm">{localBacktester.bulgariaSlippagePct}%</span>
                    </div>
                    <Slider 
                      value={[localBacktester.bulgariaSlippagePct]} 
                      min={0} max={5} step={0.1}
                      onValueChange={(val) => setLocalBacktester({...localBacktester, bulgariaSlippagePct: val[0]})}
                    />
                     <p className="text-xs text-muted-foreground">Price slippage penalty applied to backtest trades.</p>
                  </div>
                </>
              )}
            </CardContent>
            <CardFooter>
              <Button onClick={handleBacktesterSave} disabled={saving}><Save className="mr-2 h-4 w-4"/> Save Simulation Settings</Button>
            </CardFooter>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="assets">
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2"><Activity className="h-5 w-5" /> Asset Whitelist</CardTitle>
            ...
            <CardContent className="space-y-6">
              <div className="flex items-center space-x-2">
                <Switch 
                  ...
                  onCheckedChange={(checked) => setLocalWhitelist({...localWhitelist, enabled: checked})}
                />
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mt-4">
                {Object.entries(localWhitelist.assets).map(([asset, enabled]) => (
                  <div key={asset} className="flex items-center space-x-2 border p-3 rounded-md">
                    ...
                    />
                    <Label htmlFor={`asset-${asset}`} className="cursor-pointer font-mono">{asset}</Label>
                  </div>
                ))}
              </div>
            </CardContent>
            <CardFooter>
              <Button onClick={handleWhitelistSave} disabled={saving}><Save className="mr-2 h-4 w-4"/> Save Whitelist</Button>
            </CardFooter>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="system">
          <Card>
             <CardHeader>
              <CardTitle className="flex items-center gap-2"><Server className="h-5 w-5" /> System Parameters</CardTitle>
             ...
             <CardContent className="space-y-6">
                 ...
                 <>
                  <div className="space-y-2">
                    ...
                    <Input 
                      ...
                      onChange={(e) => setLocalSystem({...localSystem, rate_limit_per_sec: parseInt(e.target.value) || 10})}
                    />
                     <p className="text-xs text-muted-foreground">Max requests per second for Order Manager.</p>
                  </div>
                 </>
               )}
               ...
               {localMemory && (
                 <div className="space-y-4 pt-4 border-t">
                    <h3 className="font-semibold text-sm uppercase text-muted-foreground">Strategic Memory</h3>
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-2">
                        <Label>Max Records</Label>
                        <Input type="number" value={localMemory.maxRecords} onChange={(e) => setLocalMemory({...localMemory, maxRecords: parseInt(e.target.value) || 1000})} />
                      </div>
                      <div className="space-y-2">
                        <Label>Archive After (Days)</Label>
                        <Input type="number" value={localMemory.archiveAfterDays} onChange={(e) => setLocalMemory({...localMemory, archiveAfterDays: parseInt(e.target.value) || 90})} />
                      </div>
                    </div>
                 </div>
               )}
             ...
             <CardFooter>
              <Button onClick={handleSystemSave} disabled={saving}><Save className="mr-2 h-4 w-4"/> Save System Settings</Button>
            </CardFooter>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="connections">
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2"><Globe className="h-5 w-5" /> Exchange Connections</CardTitle>
            ...
            <CardContent className="space-y-6">
                ...
                <>
                  <div className="flex items-center justify-between p-4 border rounded-lg bg-secondary/10">
                    ...
                    <Switch
                      ...
                      onCheckedChange={(checked) => setLocalApiKeys({...localApiKeys, testnet: checked})}
                    />
                  </div>
                  
                   <div className="space-y-4">
                     <h3 className="font-semibold text-sm uppercase text-muted-foreground">Bybit Credentials</h3>
                     ...
                     <div className="space-y-2">
                       ...
                       <Input 
                         ...
                         placeholder="Masked"
                         onChange={(e) => setLocalApiKeys({...localApiKeys, bybit_api_key: e.target.value})} 
                       />
                     </div>
                     ...
                     <div className="space-y-2">
                       ...
                       <Input 
                         ...
                         placeholder="Masked"
                         onChange={(e) => setLocalApiKeys({...localApiKeys, bybit_api_secret: e.target.value})} 
                       />
                     </div>
                   </div>

                   <div className="pt-4 flex items-center gap-2">
                      <div className={`h-2 w-2 rounded-full ${localApiKeys.validated ? 'bg-green-500' : 'bg-red-500'}`} />
             ...
             <CardFooter>
              <Button onClick={handleApiKeysSave} disabled={saving} variant="default"><Save className="mr-2 h-4 w-4"/> Update Credentials</Button>
            </CardFooter>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="broker">
           <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2"><Wallet className="h-5 w-5" /> Broker Connection</CardTitle>
            ...
            <CardContent className="space-y-6">
              ...
              <div className="flex items-center justify-between rounded-lg bg-secondary p-4">
                ...
                <div className={`h-3 w-3 rounded-full ${config.api_keys.validated ? 'bg-green-500' : 'bg-red-500'}`} />
              </div>
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">Currently locked to BYBIT/MEXC in ConfigSchema.</p>
              </div>
              ...
              <div className="space-y-2">
                ...
                <Input 
                  ...
                  placeholder={config.api_keys.has_api_secret ? "••••••••••••••••" : "Enter API Secret"} 
                />
              </div>
                ...
                <div className="flex">
                  <div className="ml-3">
                    ...
                    <div className="text-sm text-yellow-500/90 mt-2">
                      <p>API Keys are stored in memory. They may be lost if the service restarts unless configured in environment variables.</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <CardFooter>
              <Button onClick={handleBrokerSave} disabled={saving || !localApiKey || !localApiSecret}>
