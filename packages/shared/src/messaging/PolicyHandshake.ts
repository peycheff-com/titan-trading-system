 ...
 */
export async function requestExecutionPolicyHash(
  nats: NatsClient,
  timeoutMs = 5000,
  retries = 3,
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      const response = await nats.request<PolicyHashResponse>(
        POLICY_HASH_REQUEST_SUBJECT,
        { request_type: 'policy_hash' },
        { timeout: timeoutMs },
      );
      ...
      console.warn(
        `Policy hash request attempt ${attempt}/${retries} failed: ${lastError.message}`,
      );
  ...
  throw new Error(
    `Failed to get policy hash from Execution after ${retries} attempts: ${lastError?.message}`,
  );
}
 ...
 */
export async function verifyExecutionPolicyHash(
  nats: NatsClient,
  localPolicyHash: string,
  timeoutMs = 5000,
  retries = 3,
): Promise<PolicyHandshakeResult> {
  try {
