...
describe('System Invariants (Living Documentation)', () => {
  describe('I-04: Symbol Whitelist', () => {
    it('symbolWhitelist exists and is non-empty in risk_policy.json', () => {
      const policyPath = join(REPO_ROOT, 'packages/shared/risk_policy.json');
      expect(existsSync(policyPath), 'risk_policy.json must exist').toBe(true);

      const policy = JSON.parse(readFileSync(policyPath, 'utf-8'));
      expect(policy.symbolWhitelist, 'symbolWhitelist field must exist').toBeDefined();
      expect(Array.isArray(policy.symbolWhitelist), 'symbolWhitelist must be array').toBe(true);
      expect(policy.symbolWhitelist.length, 'symbolWhitelist must not be empty').toBeGreaterThan(0);
    });
  });

  describe('I-02: HMAC Fail-Closed', () => {
    it('HmacValidator::new() contains panic! for empty secret', () => {
      const securityPath = join(REPO_ROOT, 'services/titan-execution-rs/src/security.rs');
      expect(existsSync(securityPath), 'security.rs must exist').toBe(true);
  ...
  describe('I-16: RiskState Enum', () => {
    it('RiskState enum defines Normal, Cautious, Defensive, Emergency', () => {
      const riskPolicyPath = join(REPO_ROOT, 'services/titan-execution-rs/src/risk_policy.rs');
      expect(existsSync(riskPolicyPath), 'risk_policy.rs must exist').toBe(true);
  ...
  describe('I-11: TokenBucket Rate Limiter', () => {
    it('TokenBucket struct exists in rate_limiter.rs', () => {
      const rateLimiterPath = join(REPO_ROOT, 'services/titan-execution-rs/src/rate_limiter.rs');
      expect(existsSync(rateLimiterPath), 'rate_limiter.rs must exist').toBe(true);
  ...
  describe('I-12: RiskGuard Evaluate', () => {
    it('RiskGuard::evaluate() exists in risk_guard.rs', () => {
      const riskGuardPath = join(REPO_ROOT, 'services/titan-execution-rs/src/risk_guard.rs');
      expect(existsSync(riskGuardPath), 'risk_guard.rs must exist').toBe(true);
  ...
  describe('I-17: Health Endpoints', () => {
    it('docker-compose.prod.yml contains healthcheck directives', () => {
      ...
      expect(existsSync(composePath), 'docker-compose.prod.yml must exist').toBe(true);
  ...
  describe('I-19: DLQ Constants', () => {
    it('DLQ_EXECUTION_CORE constant exists in subjects.rs', () => {
      const subjectsPath = join(REPO_ROOT, 'services/titan-execution-rs/src/subjects.rs');
      expect(existsSync(subjectsPath), 'subjects.rs must exist').toBe(true);
